...
- **多應用程式**：使用者可以在裝置上存取一組有限的應用程式。 當裝置啟動時，只會啟動您新增的應用程式。 您也可以新增一些使用者可開啟的網頁連結。 套用原則時，使用者會在主畫面上看到所允許之應用程式的圖示。
  ...
  - **虛擬首頁按鈕**：選擇 [啟用] 以在專用裝置上顯示首頁按鈕。 選取此按鈕時，它會讓使用者返回裝置的主畫面，讓使用者可輕鬆地在應用程式間切換。 在某些 Android 裝置上，使用者可能需要在螢幕上向上撥動以顯示主畫面按鈕。 [停用] 不會顯示主畫面按鈕，因此使用者必須使用返回按鈕以在應用程式間切換。
  ...
  - **Wi-Fi 設定**：選擇 [啟用] 以允許使用者將裝置連線到不同的 WiFi 網路。 啟用這項功能也會開啟裝置位置。 [未設定] (預設) 可防止使用者在受控主畫面 (鎖定工作模式) 中時連線到 WiFi 網路。

    [鎖定工作模式](https://developer.android.com/work/dpc/dedicated-devices/lock-task-mode)的詳細資訊 (開啟 Android 的網站)。

  - **藍牙設定**：選擇 [啟用] 以允許使用裝置上的藍牙，並允許終端使用者透過藍牙配對裝置。 啟用這項功能也會開啟裝置位置。 [未設定] (預設) 可防止使用者在受控主畫面 (鎖定工作模式) 中時設定藍牙並配對裝置。 

...
當您使用裝置設定原則來設定 BitLocker 時，您可以在 Intune 入口網站中檢查原則的狀態。 在入口網站中，移至 [**裝置**設定] [ > **設定檔**] > 選取包含 BitLocker 設定的設定檔，然後選取 [**裝置狀態**]。 系統會列出指派給設定檔的裝置，而 [*裝置狀態*] 欄會指出裝置是否已成功部署設定檔。 
...
在 BitLocker 原則成功部署至裝置之後，請在您可以在其中檢查 BitLocker 設定設定的裝置上，查看下列登錄機碼： *HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PolicyManager\current\device\BitLocker*. 範例如下：
...
- **裝置是否已正確註冊至 Microsoft Intune？** 如果不是，您必須先解決此問題，再疑難排解原則特定的任何專案。 您可以在[這裡](../enrollment/troubleshoot-windows-enrollment-errors.md)找到疑難排解 Windows 註冊問題的協助。  
- **裝置上是否有使用中的網路連線？** 如果裝置處於飛機模式或已關閉，或使用者在沒有服務的位置中有裝置，則在還原網路連線之前，不會傳送或套用原則。  

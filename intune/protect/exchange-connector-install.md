...
|  Active Directory 同步處理             | 您必須[設定 Active Directory 同步處理](../fundamentals/users-add.md)，以便將本機使用者和安全性群組與您的 Azure Active Directory 執行個體同步處理，才能使用連接器將 Intune 連線到您的 Exchange Server。 |

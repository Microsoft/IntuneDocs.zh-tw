...
- **在非受控應用程式中檢視公司文件**：[封鎖] 會防止在非受控應用程式中檢視公司文件。 [未設定] 會允許在任何應用程式中檢視公司文件。 例如，您想要防止使用者將檔案從 OneDrive 應用程式儲存至 Dropbox。 請將此設定設定為 [封鎖]。 裝置收到原則後 (例如重新啟動後)，即不再允許儲存。
  - **允許受控應用程式將連絡人寫入非受控連絡人帳戶**：當設定為 [允許] 時，使用者可以將任何人的 Outlook 連絡人資訊 (包括商務和公司連絡人) 新增或同步至裝置上內建 [連絡人] 應用程式。 當設定為 [未設定] 時，使用者無法將 Outlook 連絡人新增至裝置上的內建 [連絡人] 應用程式。
...
- **對行動數據方案設定的變更 (僅限受監督)**：[封鎖] 會防止使用者在行動數據方案中變更任何設定。 [未設定] (預設) 可讓使用者進行變更。
...
- **禁止設定新的附近裝置 (僅限受監督)**：[封鎖] 會停用設定附近新裝置的提示。 [未設定] (預設) 會允許讓使用者連線到其他附近 Apple 裝置的提示。
...
- **封鎖 iCloud 文件同步 (僅限受監督)**：[未設定] 會允許文件和索引鍵/值同步處理到 iCloud 儲存空間。 [封鎖] 會防止 iCloud 同步文件和資料。

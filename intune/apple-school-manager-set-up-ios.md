...
本主題將協助您針對透過 [Apple School Manager](https://school.apple.com/) 計劃購買的裝置設定 iOS 裝置註冊。 使用 Intune 與 Apple School Manager，您甚至不用碰到它們即可註冊大量的 iOS 裝置。 當學生或老師啟動裝置時，會以預先設定的設定來執行設定助理，並註冊裝置以接受管理。
...
3.  輸入 **MDM 伺服器名稱**。 您可參考這個伺服器名稱，以識別行動裝置管理 (MDM) 伺服器， 但它不是 Microsoft Intune 伺服器的名稱或 URL。
   ![選取序號選項的 Apple School Manager 入口網站](./media/asm-server-assignment.png)
...
6. 移至 [裝置指派]，然後手動輸入 [序號]、[訂單號碼]，或 [上傳 CSV 檔案] 來 [選擇裝置]。
     ![選取序號選項的 Apple School Manager 入口網站](./media/asm-device-assignment.png)

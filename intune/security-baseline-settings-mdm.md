...
檢視或更新版本執行 Windows 10 之裝置的支援由 Microsoft Intune 的 MDM 安全性基準設定。 此基準中設定的預設值代表針對適用的裝置，建議的設定，而可能不符合基準從其他的安全性基準的預設值。  

最新的基準版本為止**MDM Spring 2019 update （19 小時 1） 的安全性基準**  
...
- **使用提高的權限的區塊 MSI 應用程式安裝**  
  ...
  - *如果您啟用此原則設定*，權限延伸至所有程式。 這些權限通常被保留給指派給使用者 （在桌面上），指派給電腦 （自動安裝） f，或在控制台中可新增或移除程式 中的程式。 此設定檔設定可讓安裝程式需要存取目錄的使用者可能沒有檢視或變更，包括高限制性的電腦上的目錄的權限的使用者。
  - *如果您停用或未設定此原則設定*，安裝程式系統管理員不會散佈或優惠時，系統會套用目前的使用者權限。 注意：此原則設定會出現在 [電腦設定] 和 [使用者設定] 資料夾中。 若要讓此原則設定生效，您必須在這兩個資料夾中啟用它。 注意： 有經驗的使用者可以利用權限的設定變更其權限，並取得的永久存取權受限制的檔案和資料夾的授與此原則。 請注意，此原則設定的使用者組態版本不一定是安全。  
...
- **列舉與 Kernel DMA Protection 不相容的外部裝置**  
  此原則被要提供額外的安全性，對外部的 DMA 功能的裝置。 它可讓您更有效地控制與 DMA 重新對應/裝置記憶體隔離和沙箱處理不相容之外部 DMA 裝置的列舉。 只有當支援 Kernel DMA Protection 並由系統韌體啟用它時，此原則才會生效。 核心 DMA 保護是透過原則或終端使用者無法控制的平台功能。 系統製造時即必須支援此功能。 若要檢查系統是否支援核心 DMA 保護，請檢查 MSINFO32.exe 摘要頁面中的核心 DMA 保護欄位。  
...
- **Internet Explorer 加密支援**  
  此原則設定可讓您關閉瀏覽器中的傳輸層安全性 (TLS) 1.0、 TLS 1.1，TLS 1.2、 安全通訊端層 (SSL) 2.0 中或 SSL 3.0 支援。 TLS 和 SSL 會保護瀏覽器與目標伺服器之間通訊的通訊協定。 當瀏覽器嘗試設定 受保護的通訊，與目標伺服器時，瀏覽器和伺服器交涉的通訊協定和要使用的版本。 瀏覽器和伺服器嘗試比對彼此的支援的通訊協定和版本清單，並選取最常用的相符項目。 如果您啟用此原則設定時，瀏覽器會交涉，或未使用的加密方法，從下拉式清單中選取交涉的加密通道。 如果您停用或未設定此原則設定，使用者可以選取哪種加密方法瀏覽器支援。  
  [深入了解](https://go.microsoft.com/fwlink/?linkid=2067057)

  **預設**: 2 個項目： TLS 1.1 和 TLS 1.2  
  *選取向下箭號，以顯示您可以選取此設定的選項。*
...
- **請求遠端協助**  
  此原則設定可讓您開啟或關閉 Solicited (如 Ask) 這台電腦上的遠端協助。 
  - *如果您啟用此原則設定*，這部電腦上的使用者可以使用電子郵件或檔案傳輸要求的人取得協助。 此外，使用者可以使用立即訊息程式，以允許連接到這台電腦，而且您可以設定額外的遠端協助設定。 
  - *如果您停用此原則設定*，這部電腦上的使用者無法使用電子郵件或檔案傳輸要求的人取得協助。 此外，使用者無法使用立即訊息程式，以允許連接到這台電腦。 
  - *如果您未設定此原則設定*，使用者可以開啟或關閉 Solicited (如 Ask) 的遠端協助本身在控制台中的 系統內容中。 使用者也可以設定遠端協助設定。 

  如果您啟用此原則設定，您會有兩種方式可讓協助程式提供遠端協助: 「 允許 helper 來只檢視電腦 」 或 「 允許遠端控制電腦的協助程式 」。 「 最大票證時間 」 原則設定會設定限制的時間量，透過電子郵件或檔案傳輸建立遠端協助邀請可以保持開啟。 [選取傳送電子郵件邀請的方法] 設定會指定用來傳送遠端協助邀請的電子郵件標準。 根據您的電子郵件程式，您可以使用的 Mailto 標準 （邀請的收件者連線透過網際網路連結） 或 SMAPI (簡單 MAPI) 標準 （邀請附加到電子郵件訊息）。 因為 SMAPI 是支援的唯一方法，所以無法在 Windows Vista 中使用此原則設定。 如果您啟用此原則設定，也應該啟用適當的防火牆例外狀況，以允許遠端協助通訊。  
  [深入了解](https://go.microsoft.com/fwlink/?linkid=2067198)

  **預設**： 停用遠端協助

  當設定為*啟用遠端協助*，設定下列其他設定：  
  - **遠端協助請求權限**  
    **預設值**：檢視  
...
- **行為監視**  
  允許或不允許 Windows Defender 行為監視功能。 這些內建在 Windows 10 中的感應器可以收集並處理作業系統的行為訊號，並將此感應器資料傳送至您私人獨立的 Microsoft Defender ATP 雲端執行個體。  
...
如需詳細資訊，請參閱 < [2.2.2 FW_PROFILE_TYPOE]( https://docs.microsoft.com/openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc) Windows 通訊協定文件中。  
...
- **要求使用進階防詐騙 (如可使用)** ：  
  如果是，裝置會使用增強型防詐騙功能，可以使用。 如果否，將會封鎖防詐騙功能。 未設定，則會接受用戶端上完成設定。  
...
- **PIN 中需要有小寫字母**  
  如有需要，使用者 PIN 必須包含至少一個小寫字母。

  **預設值**：允許

- **PIN 中需要有特殊字元**  
  如有需要，使用者 PIN 必須包含至少一個特殊字元。

  **預設值**：允許

- **PIN 長度下限**  
  最小 PIN 長度必須介於 4 到 127 之間。

  **預設值**：6

- **PIN 中需要有大寫字母**  
  如有需要，使用者 PIN 必須包含至少一個大寫字母。
...
## <a name="whats-changed-in-the-new-template"></a>在新的範本中變更的項目
*MDM Spring 2019 update （19 小時 1） 的安全性基準*範本具有下列變更，從*預覽*範本。

### <a name="changes-to-the-baseline-settings"></a>基準設定的變更
下列設定是：
- *新*中最新版的基準。
- *移除*這個最新的基準版本，但已存在於上一個版本。
...
*[修訂]* [ **Internet Explorer**](#internet-explorer):
- **Internet Explorer 網際網路區域自動提示下載檔案**> 的預設值現在是**已停用**。 在預覽中，這是設定為 已啟用。

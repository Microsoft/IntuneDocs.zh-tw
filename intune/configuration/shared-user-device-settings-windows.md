...
這些設定會使用[SHAREDPC CSP](https://docs.microsoft.com/windows/client-management/mdm/sharedpc-csp)。
...
- **快速第一次登入**：選擇 [**已啟用**]，讓使用者有快速的第一次登入體驗。 **啟用**時，裝置會自動將新的非系統管理員 Azure AD 帳戶連接到預先設定的候選本機帳戶。 選擇 [**停用**] 以防止快速第一次登入體驗。 [未設定]  (預設) 會保留這項設定不受 Intune 管理，且不會推送任何原則來控制裝置上的這項設定。

...
titleSuffix: ''
description: 將 Microsoft Intune 與 Saaswedo 電信費用管理服務整合來監視數據使用量，並在 Android 和 iOS 裝置上設定閾值或限制。
keywords: Saaswedo
...
ms.locfileid: "71723849"
---
# <a name="set-up-a-telecom-expense-management-service-in-intune"></a>在 Intune 中設定電信費用管理服務

[!INCLUDE [azure_portal](../includes/azure_portal.md)]

使用 Intune，您可以在組織擁有的行動裝置上管理來自數據使用量的電信費用。 Intune 會與 Saaswedo 的 [Datalert 電信費用管理](http://datalert.biz/get-started) \(英文\) 整合。 Datalert 是可管理電信數據使用量的即時電信費用管理解決方案。 這可協助受 Intune 管理的裝置避免產生高額與預期外的數據和漫遊費用。
...
若要搭配 Intune 使用 Datalert 服務，Datalert 和 Intune 中有一些組態設定。 本文將示範下列項目的作法：

- 設定 Datalert 主控台中的設定，以將 Datalert 服務連線至 Intune。
- 確認此連線處於作用中狀態且已在 Intune 中啟用。
- 使用 Intune 來將 Datalert 應用程式新增至裝置。
- 關閉 Datalert 服務和 Intune (選擇性)。

## <a name="supported-platforms"></a>支援的平台

- 具備 Knox 功能的 Android 4.4 和更新版本裝置 (Samsung)

  [支援 Knox 的 Android 版本](https://seap.samsung.com/faq/what-versions-android-support-knox-standard-and-knox-premium-sdks-0) \(英文\) (開啟 Samsung 的網站) 會列出 Knox 支援的版本。

- iOS 8.0 和更新版本

## <a name="prerequisites"></a>必要條件

- Microsoft Intune 的訂用帳戶，以及存取 [Azure 入口網站](https://portal.azure.com)的權限
...
## <a name="deploy-the-intune-and-datalert-solution"></a>部署 Intune 和 Datalert 解決方案

### <a name="step-1-connect-the-datalert-service-to-intune"></a>步驟 1：將 Datalert 服務連線到 Intune
...
5. 針對 [Azure AD domain]  \(Azure AD 網域\)，輸入您的 Azure 租用戶識別碼。 選取 [Connection]  \(連線\)。
    ...
    當您選取 [Connection]  \(連線\) 時，Datalert 服務會使用 Intune 簽入。 它會確認目前沒有任何 Datalert 連線。 Microsoft 登入頁面會在片刻之後出現，然後是 Datalert Azure 驗證。
...
#### <a name="create-device-categories-and-device-groups-mapped-to-your-categories"></a>建立裝置類別及對應至類別的裝置群組
...
下列步驟會新增 Datalert 應用程式。 例如，使用 iOS。 [新增應用程式](../apps/apps-add.md)和[使用範圍標籤](../fundamentals/scope-tags.md)提供更多有關這些步驟的具體資訊。

1. 在 **[Intune](https://go.microsoft.com/fwlink/?linkid=2090973)** 中，選取 [用戶端應用程式]   > [應用程式]   > [新增]  。

2. 選取您的 [應用程式類型]  。 例如，針對 iOS，選取 [商店應用程式 - iOS]  。
...
1. 在 [用戶端應用程式 - 應用程式]  中，選取您在上一個步驟中新增的 Datalert 應用程式。
...
Intune 和 Datalert 服務現在會設定為彼此通訊。 接下來，將組織付費電話線路新增至 Datalert 主控台。 然後，輸入任何行動數據或漫遊使用量違規的閾值和動作。 您可以手動將公司付費電話線路新增至 Datalert 主控台，或是讓線路在裝置註冊到 Intune 後自動新增。
...
1. 在 **[Intune](https://go.microsoft.com/fwlink/?linkid=2090973)** 中，選取 [裝置設定]   > [電信費用管理]  。
...
> - 使用者不再受限於數據存取與漫遊。
> - Intune 仍會接收來自服務的訊號，但 Intune 會忽略這類訊號。

...
- **螢幕擷取**：選擇 [封鎖]  來防止在裝置上取得螢幕擷取畫面或進行畫面擷取。 在 iOS 9.0 和更新版本中，它也會封鎖螢幕錄製。 [未設定]  (預設) 可讓使用者將螢幕內容擷取為影像或影片。
...
- **修改診斷提交設定**：[封鎖]  可防止使用者變更 [診斷及使用方式]  (裝置 [設定]) 中的診斷提交與應用程式分析設定。 [未設定]  (預設) 允許使用者變更這些裝置設定。
...
- **教室應用程式進行遠端螢幕觀察**：選擇 [封鎖]  來防止教室應用程式從遠端檢視裝置上的畫面。 [未設定]  (預設) 可讓 Apple Classroom 應用程式檢視畫面。

  若要使用此設定，請將**螢幕**快照設定設為 [**封鎖**]。

  本功能適用於：  
  - iOS 9.3 與更新版本

- **「課堂」應用程式的無提示螢幕畫面觀看**：若設定為 [允許]  ，老師可以在學生不知情的情況下，使用 Classroom 應用程式觀察學生的 iOS 裝置畫面。 使用 [Classroom] 應用程式註冊至課程的學生裝置，會自動將權限授與該課程的教師。 [未設定]  (預設) 會防止此功能。

  若要使用此設定，請將**螢幕**快照設定設為 [**封鎖**]。

- **企業應用程式信任**：選擇 [封鎖]  來移除裝置上的 [設定] > [一般] > [描述檔與裝置管理] 中的 [信任企業開發人員]  按鈕。 [未設定]  (預設) 可讓使用者選擇信任不是從 App Store 下載的應用程式。
- **帳戶修改**：設定為 [封鎖]  時，使用者將無法從 iOS 設定應用程式更新裝置特定的設定。 例如，使用者無法建立新的裝置帳戶，或是變更使用者名稱或密碼。 [未設定]  (預設) 可讓使用者變更這些設定。
...
- **螢幕使用時間**：選擇 [封鎖]  以防止使用者在 [螢幕使用時間] \(裝置設定\) 中設定自己的限制。 [未設定]  可允許使用者在裝置上設定裝置限制 (例如家長監護或內容及隱私權限制)。
  ...
  - iOS 11.4.1 及較舊版本：[封鎖]  可防止終端使用者在裝置設定中設定自己的限制。 行為是相同的；對終端使用者而言沒有任何變更。
...
- **裝置上使用清除所有內容及設定的選項**：選擇 [封鎖]  來讓使用者無法在裝置上使用 [清除所有內容及設定] 選項。 [未設定]  (預設) 可讓使用者存取這些設定。
- **修改裝置名稱**：選擇 [封鎖]  來讓裝置名稱無法被變更。 [未設定]  (預設) 允許使用者變更裝置的名稱。
- **修改通知設定**：選擇 [封鎖]  來讓通知設定無法被變更。 [未設定]  (預設) 允許使用者變更裝置通知設定。
- **更換背景圖片**：[封鎖]  可防止背景圖片被變更。 [未設定]  (預設) 允許使用者變更裝置的背景圖案。
- **修改企業應用程式信任設定**：[封鎖]  可防止使用者變更受監督裝置上的企業應用程式信任設定。 [未設定]  (預設) 允許使用者信任不是從 App Store 下載的應用程式。
...
- **允許教室鎖定應用程式，以及在不顯示提示的情況下鎖定裝置**：[啟用]  會允許老師使用教室應用程式鎖定應用程式或鎖定裝置，而不提示學生。 鎖定應用程式表示裝置只能存取老師指定的應用程式。 [未設定]  (預設) 會防止老師使用教室應用程式鎖定應用程式或裝置，而不提示學生。

  本功能適用於：  
  - iOS 11.0 與更新版本

- **在不顯示提示的情況下自動加入教室班級**：[啟用]  會自動允許學生加入在教室應用程式中的班級，而不會提示老師。 [未設定]  (預設) 會提示老師學生想要加入教室應用程式中的班級。

  本功能適用於：  
  - iOS 11.0 與更新版本

- **封鎖 VPN 建立**：[封鎖]  可防止使用者建立 VPN 組態設定。 [未設定]  (預設) 可讓使用者在裝置上建立 VPN。
- **修改 eSIM 設定**：[封鎖]  可防止使用者移除或新增行動數據方案到裝置上的 eSIM。 [未設定]  (預設) 可讓使用者變更這些設定。

  本功能適用於：  
  - iOS 12.1 與更新版本

- **延遲軟體更新**：當設定為 [未設定]  (預設) 時，當 Apple 發行軟體更新時便會顯示在裝置上。 例如，如果 iOS 更新由 Apple 在特定日期發行，則該更新會在發行日期左右自然地出現在裝置上。
...
- **密碼**：**要求**使用者輸入密碼才可存取該裝置。 [未設定]  \(預設\) 可讓使用者存取裝置，而不需要輸入密碼。
...
- **密碼最小長度**：輸入使用者必須輸入的最小長度 (介於 4 到 14 個字元之間)。 在 [使用者註冊的裝置] 上，輸入介於4到6個字元的長度。
  
  > [!NOTE]
  > 對於已註冊使用者的裝置，使用者可以設定大於6位數的 PIN。 但在裝置上不會強制執行6個以上的數位。 例如，系統管理員將最小長度設定為 `8`。 在使用者註冊的裝置上，使用者只需要設定6位數的 PIN。 Intune 不會在使用者註冊的裝置上強制 PIN 大於6位數。

- **登入失敗幾次後即抹除裝置**：輸入抹除裝置前允許的登入失敗次數 (介於 4-11 次)。
...
- **TOUCH id 和臉部識別碼解除鎖定**：選擇 [**封鎖**] 以防止使用指紋或臉部來解除鎖定裝置。 [未設定]  會允許使用者使用這些方法將裝置解除鎖定。
  ...
  封鎖這種設定也會導致無法使用 FaceID authentication 來解除鎖定裝置。
...
- **修改密碼**：選擇 [封鎖]  來防止變更、新增或移除密碼。 在封鎖此功能之後，系統會忽略在受監督裝置上對密碼限制所做的變更。 [未設定]  (預設) 允許新增、變更或移除密碼。

  - **TOUCH id 和臉部識別碼修改**： [**封鎖**] 會阻止使用者變更、新增或移除 TOUCHID 指紋和臉部識別碼。 [未設定]  \(預設\) 允許使用者更新裝置上的 TouchID 指紋和 Face ID。

    封鎖這種設定也會阻止使用者變更、新增或移除 FaceID authentication。

    臉部識別碼適用于：  
    - iOS 11.0 與更新版本

- **防止自動填滿密碼**：選擇 [封鎖]  來防止使用 iOS 上的自動填滿密碼功能。 選擇 [封鎖]  也有下列影響：
...
<sup>1</sup>當您設定 [沒有活動最久幾分鐘後鎖定螢幕]  及 [在螢幕鎖定最少幾分鐘後必須輸入密碼]  設定時，它們會依序套用。 例如，如果您將兩個值都設定為 **5** 分鐘，則系統會在五分鐘後自動關閉螢幕，然後等候額外五分鐘才鎖定裝置。 但使用者若是手動關閉螢幕，便會立即套用第二項設定。 在同一範例中，當使用者關閉螢幕之後，裝置會在五分鐘後鎖定。

## <a name="locked-screen-experience"></a>鎖定畫面體驗

### <a name="settings-apply-to-all-enrollment-types"></a>設定適用于：所有註冊類型

- **在鎖定裝置時存取控制中心**：選擇 [封鎖]  以防止在裝置鎖定時存取 [控制中心] 應用程式。 [未設定]  \(預設\) 會允許使用者在裝置鎖定時存取 [控制中心] 應用程式。
- **在鎖定裝置時使用通知**：[封鎖]  會防止在裝置鎖定時存取通知。 [未設定]  \(預設\) 會允許使用者在未解除鎖定裝置的情況下存取通知。
- **在鎖定裝置時使用今日檢視**：[封鎖]  會防止在裝置鎖定時存取今日檢視。 [未設定]  \(預設\) 會允許使用者在裝置鎖定時存取今日檢視。

### <a name="settings-apply-to-device-enrollment-automated-device-enrollment-supervised"></a>設定適用于：裝置註冊、自動裝置註冊（受監督）

- **裝置鎖定住時的電子錢包通知**：[封鎖]  會防止在裝置鎖定時存取 [錢包] 應用程式。 [未設定]  \(預設\) 會允許使用者在裝置鎖定時存取 [錢包] 應用程式。
...
- **分級區域**：選擇您想要針對允許的下載項目使用的分級區域。 然後選擇適用於**電影**、**電視節目**和**應用程式**的允許分級。

### <a name="settings-apply-to-automated-device-enrollment-supervised"></a>設定適用于：自動裝置註冊（受監督）

- **App Store**：[封鎖]  會防止受監督裝置上對 App Store 的存取。 [未設定]  (預設) 允許存取。

  從 iOS 13.0 開始，此設定需要受監督的裝置。

  - **從 App Store 安裝應用程式**：選擇 [封鎖]  來從裝置主畫面封鎖 App Store。 終端使用者可以繼續使用 iTunes 或 Apple Configurator 來安裝應用程式。 [未設定]  \(預設\) 會在主畫面上允許 App Store。
  - **應用程式自動下載**：選擇 [封鎖]  來防止自動下載在其他裝置上購買的應用程式。 這不會影響對現有應用程式的更新。 [未設定]  \(預設\) 會允許在裝置上下載於其他 iOS 裝置上購買的應用程式。

- **偏激的 iTunes 音樂、播客或新聞內容**：選擇 [封鎖]  以防止偏激的 iTunes 音樂、播客或新聞內容。 [未設定]  \(預設\) 會允許裝置存取市集中評分為成人的內容。 iOS 13 和更新版本可能需要受監督的裝置。 

  從 iOS 13.0 開始，此設定需要受監督的裝置。

- **正在新增 Game Center 的朋友**：[封鎖]  可防止使用者新增 Game Center 朋友。 [未設定]  \(預設\) 會允許使用者在 [Game Center] 中新增朋友。

  從 iOS 13.0 開始，此設定需要受監督的裝置。

- **Game Center**：[封鎖]  使用 [Game Center] 應用程式。 [未設定]  \(預設\) 會允許在裝置上使用 [Game Center] 應用程式。
- 多**人遊戲**：選擇 [**封鎖**] 以防止多人遊戲。 [未設定]  \(預設\) 會允許使用者在裝置上進行多人遊戲。
...
- **Siri**：[封鎖]  會防止存取 Siri。 [未設定]  \(預設\) 會允許在裝置上使用 Siri 語音助理。
  - **在鎖定裝置時使用 Siri**：選擇 [封鎖]  來防止在裝置鎖定時存取 Siri。 [未設定]  \(預設\) 會允許在裝置鎖定時使用 Siri 語音助理。

- **Safari 詐騙警告**：[需要]  會在裝置上的網頁瀏覽器中顯示詐騙警告。 [未設定]  (預設) 會停用此功能。
...
- **相機**：選擇 [封鎖]  來防止在裝置上存取相機。 [未設定]  (預設) 會允許存取裝置的相機。

  從 iOS 13.0 開始，此設定需要受監督的裝置。

  - **FaceTime**：選擇 [封鎖]  來防止存取 [FaceTime] 應用程式。 [未設定]  \(預設\) 會允許存取裝置上的 [FaceTime] 應用程式。
...
- **Siri 從網際網路查詢使用者產生的內容**：[封鎖]  可防止 Siri 存取網站來回答問題。 [未設定]  \(預設\) 可允許 Siri 從網際網路存取使用者產生的內容。

  若要使用此設定，請將**Siri**設定設為 [**封鎖**]。

- **Apple News**：選擇 [封鎖]  可防止在裝置上存取 [Apple News] 應用程式。 [未設定]  \(預設\) 可允許使用 [Apple News] 應用程式。
- **iBooks Store**：[封鎖]  可防止存取 iBooks Store。 [未設定]  \(預設\) 可允許使用者從 iBooks Store 瀏覽及購買書籍。
- **裝置上的訊息應用程式**： [**封鎖**] 會防止使用者使用 [訊息] 應用程式進行 iMessage。 如果裝置支援文字訊息，則使用者仍然可以使用 SMS 來傳送和接收文字訊息。 [**未設定**] （預設）允許使用 [訊息] 應用程式透過網際網路傳送和讀取訊息。
- **Podcast**：[封鎖]  可防止使用者使用 [Podcast] 應用程式。 [未設定]  \(預設\) 可允許使用 [Podcast] 應用程式。
- **音樂服務**：[封鎖]  會將 [音樂] 應用程式還原至傳統模式，並停用音樂服務。 [未設定]  (預設) 會允許使用 Apple Music 應用程式。
- **iTunes Radio 服務**：[封鎖]  可防止使用者使用 [iTunes Radio] 應用程式。 [未設定]  \(預設\) 可允許使用 [iTunes Radio] 應用程式。
- **itunes store**： [**未設定**] （預設）允許裝置上的 iTunes。 [**封鎖**] 會防止使用者在裝置上使用 iTunes。 

  本功能適用於：  
  - iOS 4.0 與更新版本

- [**尋找我的 iPhone**：**未設定**] （預設）允許使用此 [尋找我的應用程式] 功能來取得裝置的近似位置。 [**封鎖**] 會在 [尋找我的應用程式] 中防止此功能。 

  本功能適用於：  
  - iOS 13.0 和 iPadOS 13.0 及更新版本

- **尋找我的朋友**： [**未設定**] （預設）允許使用此 [尋找我的應用程式] 功能，從 Apple 裝置或 iCloud.com 尋找家庭和朋友。 [**封鎖**] 會在 [尋找我的應用程式] 中防止此功能。

  本功能適用於：  
  - iOS 13.0 和 iPadOS 13.0 及更新版本

- **對尋找我的朋友應用程式設定的變更**：[封鎖]  可防止對 [尋找我的朋友] 應用程式設定進行變更。 [未設定]  \(預設\) 可允許使用者變更 [尋找我的朋友] 應用程式的設定。

- **Spotlight 搜尋傳回網際網路上的結果**：[封鎖]  可防止 Spotlight 傳回來自網際網路搜尋的任何結果。 [未設定]  (預設) 會允許 Spotlight 搜尋連線到網際網路以提供搜尋結果。

- **禁止從裝置移除系統應用程式**：選擇 [封鎖]  可停用從裝置移除系統應用程式的能力。 [未設定]  \(預設\) 可允許使用者移除系統應用程式。

- **Safari**：[封鎖]  在裝置上使用 Safari 瀏覽器。 [未設定]  \(預設\) 可允許使用者使用 Safari 瀏覽器。

  從 iOS 13.0 開始，此設定需要受監督的裝置。

- **Safari 自動填入**：[封鎖]  可停用裝置上 Safari 中的自動填入功能。 [未設定]  (預設) 會允許使用者變更網頁瀏覽器中的自動完成設定。
...
- **新增**：選取以建立您的應用程式清單。
- **匯入**具有應用程式詳細資料 (包括其 URL) 的 CSV 檔案。 請使用 `<app url>, <app name>, <app publisher>` 格式。 或者，**匯出**以使用相同的格式來建立您所新增之受限制應用程式的清單。
...
- **變更應用程式行動數據使用量設定**：選擇 [封鎖]  來防止對應用程式行動數據使用量設定進行變更。 [未設定]  (預設) 允許使用者控制有哪些應用程式可以使用行動數據。
- **對行動數據方案設定的變更**：[封鎖]  可防止使用者在行動數據方案中變更任何設定。 [未設定]  (預設) 可讓使用者進行變更。

  本功能適用於：  
  - iOS 11.0 與更新版本

- **個人熱點的使用者修改**：當設定為 [**封鎖**] 時，使用者將無法變更 [個人熱點] 設定。 [**未設定**] （預設）可讓終端使用者啟用或停用其個人熱點。

  如果您封鎖此設定並封鎖**個人熱點**設定，則會關閉個人熱點。
...
- **AirDrop**：[封鎖]  可防止在裝置上使用 AirDrop。 [未設定]  (預設) 允許使用 AirDrop 功能與鄰近裝置交換內容。
- **Apple Watch 配對**： [**封鎖**] 會防止與 Apple Watch 配對。 [未設定]  (預設) 允許裝置與 Apple Watch 配對。
- **藍牙修改**：[封鎖]  可防止使用者變更裝置上的藍牙設定。 [未設定]  (預設) 允許使用者變更這些裝置設定。
- **主機配對，以控制 iOS 裝置所能配對的裝置**：[未設定]  \(預設\) 可允許主機配對，來讓系統管理員控制能與 iOS 裝置配對的裝置。 [封鎖]  會防止主機配對。
- **封鎖 AirPrint**：選擇 [封鎖]  以防止在裝置上使用 AirPrint 功能。 [未設定]  (預設) 允許使用者使用 AirPrint。
  - **封鎖鑰匙圈中 AirPrint 認證的儲存體**：[封鎖]  可防止在裝置上使用 [鑰匙圈] 儲存體來儲存使用者名稱和密碼。 [未設定]  (預設) 允許在 [鑰匙圈] 應用程式中儲存 AirPrint 使用者名稱和密碼。
  - **AirPrint 需要信任的 TLS 憑證**：[需要]  可強制裝置針對 TLS 列印通訊使用受信任的憑證。
  - **封鎖 AirPrint 印表機的 iBeacon 探索**：[封鎖]  可防止惡意 AirPrint 藍牙指標對網路流量進行網路釣魚。 [未設定]  (預設) 允許在裝置上公告 AirPrint 印表機。
...
- **鍵盤拼寫檢查**： [**未設定**] （預設）允許在裝置上使用 spellchecker。 [封鎖]  會防止使用拼字檢查。
- **鍵盤快速鍵**： [**未設定**] （預設）允許在裝置上使用鍵盤快速鍵。 [封鎖]  會防止使用者使用鍵盤快速鍵。
- **聽寫**：[封鎖]  可防止使用者使用語音輸入來輸入文字。 [未設定]  (預設) 會允許使用者使用聽寫輸入。
- **QuickPath**： [**未設定**] （預設）可讓使用者使用 QuickPath，以允許在裝置的鍵盤上進行連續輸入。 使用者可以透過跨金鑰進行輕刷來建立文字。 [**封鎖**] 會防止使用者使用 QuickPath。 
...
- **加密備份**：[需要]  來加密所有裝置備份。
- **受控應用程式同步至雲端**：[未設定]  \(預設\) 可讓您使用 Intune 管理的應用程式將資料同步至使用者的 iCloud 帳戶。 [封鎖]  會防止針對 iCloud 進行此資料同步。
- **封鎖企業書籍備份**：選擇 [封鎖]  來防止使用者備份企業書籍。 [未設定]  \(預設\) 可讓使用者備份這些書籍。
- **封鎖企業書籍中繼資料同步 (備註與醒目提示)** ：[封鎖]  會防止同步企業書籍中的備註與醒目提示。 [**未設定**] （預設）會允許同步處理。
...
- **禁止 iCloud 文件同步**：[未設定]  \(預設\) 允許文件和機碼值同步處理到 iCloud 儲存空間。 [封鎖]  會防止 iCloud 同步文件和資料。
...
- **語音控制**： [**需要**] 可在裝置上啟用語音控制，並可讓使用者使用 Siri 命令來完全控制 OS。 [**未設定] 會**停用裝置上的語音控制。
  ...
  > [!TIP]
  > 如果您的組織可以使用 LOB 應用程式，而且在 iOS 13.0 發行時，不會在第0天準備好**語音控制**，建議您將此設定保留為 [**未設定**]。

- **VoiceOver**：[需要]  在裝置上提供 [VoiceOver] 協助工具設定，以朗讀畫面上的文字。 [未設定]  不會在 kiosk 模式中執行或啟用此功能。
- **縮放**：[需要]  在裝置上提供 [縮放] 設定，來讓使用者能使用觸控來放大畫面。 [未設定]  不會在 kiosk 模式中執行或啟用此功能。
- **自動鎖定**： [**封鎖**] 會防止裝置的自動鎖定。 [未設定]  會允許此功能。
- **響鈴開關**：[封鎖]  可停用裝置上的響鈴 (靜音) 開關。 [未設定]  會允許此功能。
- **旋轉螢幕**：[封鎖]  可防止使用者旋轉裝置時變更螢幕方向。 [未設定]  會允許此功能。
- **螢幕睡眠按鈕**：選擇 [封鎖]  以停用裝置上的螢幕睡眠喚醒按鈕。 [未設定]  會允許此功能。
- **觸控**：[封鎖]  會停用裝置上的觸控式螢幕。 [未設定]  會允許使用者使用觸控式螢幕。
- **音量按鈕**： [**封鎖**] 會防止在裝置上使用音量按鈕。 [**未設定] 會**允許音量按鈕。
...
- **語音控制項修改**：**允許**使用者變更其裝置上的語音控制狀態。 [**未設定] 會**禁止使用者變更其裝置上的語音控制狀態。

  這項設定適用於：  
  - iOS 13.0 與更新版本
  - iPadOS 13.0 和更新版本

- **VoiceOver 控制**：[允許]  VoiceOver 變更來讓使用者更新 VoiceOver 功能，例如朗讀畫面上文字的速度。 [未設定]  會防止 VoiceOver 變更。

...
- **螢幕擷取**：選擇 [封鎖]  以防止在裝置上取得螢幕擷取畫面或進行畫面擷取。 在 iOS 9.0 和更新版本中，它也會封鎖螢幕錄製。 [未設定]  (預設) 可讓使用者將螢幕內容擷取為影像或影片。
...
- **封鎖無線 PKI 更新**： [**封鎖**] 會防止您的使用者接收軟體更新，除非裝置已連線到電腦。 [**未設定**] （預設）：允許裝置在不連線至電腦的情況下接收軟體更新。
...
- **允許裝置鎖定時使用 usb 配件**： [**允許**] 可讓 usb 配件與已鎖定且已超過一小時的裝置交換資料。 [**未設定**] （預設）不會更新裝置上的 USB 限制模式，如果鎖定超過一小時，則會封鎖 USB 配件從裝置傳送資料。
...
- **密碼長度下限**：輸入使用者必須輸入的長度下限，介於 4 到 14 個字元之間。 在 [使用者註冊的裝置] 上，輸入介於4到6個字元的長度。
...
- **偏激的 iTunes 音樂、播客或新聞內容**：選擇 [封鎖]  可防止偏激的 iTunes 音樂、播客或新聞內容。 [未設定]  \(預設\) 會允許裝置存取市集中評分為成人的內容。 iOS 13 和更新版本可能需要受監督的裝置。 
...
- **Safari JavaScript**：[封鎖]  會防止在裝置上執行瀏覽器中的 JavaScript。 [**未設定**] （預設）允許 JAVA 腳本。

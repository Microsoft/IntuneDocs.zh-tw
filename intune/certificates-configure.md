...
2. 在 **Microsoft Intune** 中，選取 [裝置設定]，並選取 [設定檔]， 然後選取 [建立設定檔]。
3. 輸入受信任憑證設定檔的 [名稱] 和 [描述]。
4. 對於 [平台]，選取此受信任憑證的裝置平台： 
...
5. 對於 [設定檔類型]，選擇 [受信任的憑證]。 瀏覽至您先前儲存的憑證 (*CertificateName*.cer) (步驟 2)。

...
若為無使用者親和性的裝置，註冊後立即進行的同步處理頻率則為數小時到一或多天不等。 Intune 會以不同的時間間隔傳送要求，讓裝置簽入服務。 不過，仍會以裝置實際執行作業的時間為主。 初始註冊之後，根據裝置註冊類型以及指派給裝置的原則和設定檔會有所不同，因此無法預測裝置完成簽入所需的時間。 不過，一旦裝置完成註冊，且已套用所有初始原則，裝置應該會約每六小時檢查一次新原則。
...
當您刪除設定檔，或從群組中移除已經指派設定檔的裝置時，會根據以下清單，從裝置移除該設定檔及設定。

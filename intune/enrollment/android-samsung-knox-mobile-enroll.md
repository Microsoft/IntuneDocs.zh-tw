...
> [!NOTE]
>
>使用者關聯只適用於 Android 裝置管理員註冊。 在定義使用者關聯的情況下，只有已關聯的使用者可以使用 KME 註冊裝置。 就算是對裝置進行原廠重設也是如此。 在 Knox 入口網站中沒有定義任何使用者關聯的情況下，任何具備有效 Intune 授權的使用者都可以使用 KME 來註冊裝置。

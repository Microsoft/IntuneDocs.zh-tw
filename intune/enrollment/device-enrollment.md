...
Apple 裝置註冊計劃 (DEP) 管理功能可讓您建立原則，並將原則「以無線方式」部署至透過 DEP 購買及管理的 iOS/iPadOS 與 macOS 裝置。 當使用者第一次開啟裝置並執行「設定輔助程式」時，即會註冊裝置。 此方法支援 iOS/iPadOS 受監管模式，這樣能讓您使用特定功能來設定裝置。

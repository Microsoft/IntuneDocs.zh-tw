...
當您在 Intune 中設定適用於 Windows 10 Update 更新通道的設定時，您設定的是 Windows Update 設定。  當 Windows Update 設定具有 Windows 10 版本相依性時，版本相依性會在設定詳細資料中註明。  
...
例如，假設更新通道具有 20 天的功能更新解除安裝期間。 在 25 天之後，您決定要復原上次的功能更新，並使用 [解除安裝] 選項。  已安裝功能更新超過 20 天的裝置無法解除安裝，因為它們已在維護當中移除了必要的位元。 不過，僅安裝功能更新 19 天的裝置，只要它們在尚未超過 20 天的解除安裝期間成功簽入以接收解除安裝命令，就可以將更新解除安裝。  
...
如需此選項的詳細資訊，請參閱 Windows 10 文件中的[預約重新啟動](https://docs.microsoft.com/en-us/windows/deployment/update/waas-restart#engaged-restart) \(機器翻譯\) 以部署更新。  

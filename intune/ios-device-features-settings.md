...
1. 在 [設定] 中，選取 [AirPrint]。 輸入下列 AirPrint 伺服器屬性：
    ...
    - **TLS**：選擇 [啟用] 以透過傳輸層安全性 (TLS) 來保護 AirPrint 連線。 適用於 iOS 11.0 和更新版本。

2. 選取 [新增]。 AirPrint 伺服器隨即新增至清單中。 您可以新增許多 AirPrint 伺服器。
...
2. 在 [終端機] 中，輸入 `ippfind`，然後選取 Enter 鍵。
    ...
    請記下印表機資訊。 例如，系統可能會傳回類似 `ipp://myprinter.local.:631/ipp/port1` 的內容。 第一個部分是印表機的名稱。 最後一個部分 (`ipp/port1`) 是資源路徑。

3. 在 [終端機] 中，輸入 `ping myprinter.local`，然後選取 Enter 鍵。

   請記下 IP 位址。 例如，系統可能會傳回類似 `PING myprinter.local (10.50.25.21)` 的內容。
...
1. 在 [設定] 中，選取 [共用裝置設定 (僅限受監督)]。
2. 輸入下列設定：

    - **資產標籤資訊**：輸入裝置資產標籤的相關資訊。 例如，輸入 `Owned by Contoso Corp`。 

      您輸入的文字會顯示在裝置的登入視窗與鎖定畫面上。

    - **鎖定畫面註腳**：輸入當裝置遺失或遭竊時，可能有助於取回裝置的備註。 例如，輸入類似 `If found, call Contoso at ...` 的內容。

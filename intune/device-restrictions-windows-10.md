...
- **Ink 工作區** - 禁止使用者存取 Ink 工作區。 設定為 [未設定] 時，會啟用 Ink 工作區 (功能已開啟)，並允許使用者在鎖定畫面上使用它。
...
- **發佈使用者活動**：[封鎖] 以防止共用體驗以及在工作切換器中探索最近使用的資源。
- **僅限本機活動**：[封鎖] 以防止共用體驗，以及僅根據本機活動，在工作切換器中探索最近使用的資源。
...
- **工作** - 定義此應用程式能否存取您的工作。
- **信任的裝置** - 選擇此應用程式是否可以使用信任的裝置 (您已連線或此電腦、平板電腦或手機上搭載的硬體)。 例如電視、投影機等等。
...
-   **Microsoft Edge 瀏覽器 (僅限行動裝置版)** - 允許在裝置上使用 Microsoft Edge 網頁瀏覽器。
-   **網址列下拉式清單 (僅限桌面版)** - 阻止 Microsoft Edge 在您鍵入時，於下拉式清單中顯示建議清單。 此選項有助於將 Microsoft Edge 與 Microsoft 服務之間的網路頻寬用量降到最低。
-   **在 Microsoft 瀏覽器之間同步我的最愛 (僅限桌面版)** - 可讓 Windows 在 Internet Explorer 與 Microsoft Edge 之間同步我的最愛。
...
-   **開發人員工具** - 防止終端使用者開啟 Microsoft Edge 開發人員工具。
-   **延伸模組** - 允許終端使用者在裝置上安裝 Microsoft Edge 延伸模組。
-   **InPrivate 瀏覽** - 防止使用者開啟 InPrivate 瀏覽工作階段。
-   **顯示初次執行網頁** - 第一次執行 Microsoft Edge 時，停止顯示簡介頁面。
    -   **初次執行 URL** - 指定使用者第一次執行 Microsoft Edge 時顯示的網頁 URL (僅限 Windows 10 行動裝置版)。
-   **首頁** - 新增要作為 Microsoft Edge 瀏覽器首頁使用的網站清單 (僅限桌面版)。
-   **起始畫面的變更** - 可讓使用者變更 Microsoft Edge 開啟時顯示的起始畫面。 若要建立 Microsoft Edge 啟動時開啟的網頁或網頁清單，請使用 [首頁] 設定。
-   **禁止存取 About 旗標** - 防止終端使用者存取 Microsoft Edge 中包含開發人員和實驗性設定的 about:flags 頁面。
-   **WebRtc localhost IP 位址** - 禁止於使用 Web RTC 通訊協定撥打電話時，顯示使用者的 localhost IP 位址。
-   **預設搜尋引擎** - 指定要使用的預設搜尋引擎。 使用者可以隨時變更此值。
-   **在結束時清除瀏覽資料** - 當使用者結束 Microsoft Edge 時，清除歷程記錄和瀏覽資料。
-   **動態磚資料收集** - 當使用者從 Microsoft Edge 釘選網站到 [開始] 功能表時，阻止 Windows 從動態磚收集資訊。
-  **我的最愛清單** - 定義我的最愛檔案的路徑。 例如， http://contoso.com/favorites.html 。
-  **限制我的最愛變更**  -  **封鎖** 以防止使用者新增、匯入、排序或編輯我的最愛清單。 
...
- **從工作列取消釘選應用程式** - 選擇 [封鎖] 以阻止使用者從 [開始] 功能表取消釘選應用程式。

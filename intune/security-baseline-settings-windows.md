...
- **不可匯出認證的遠端主機委派**  
  遠端主機允許委派不可匯出認證。 當使用認證委派，裝置會提供認證可匯出版本至公開給認證遭竊的風險的使用者，從遠端主機上的攻擊者的遠端主機。 如果您啟用此原則設定，主機會支援「受限的系統管理」或「遠端 Credential Guard」模式。 如果您停用或未設定此原則設定，則不支援「受限的系統管理」和「遠端 Credential Guard」模式。 使用者一律必須將他們的認證傳遞給主應用程式。  
...
- **需要密碼**  
  ...
  **預設**： 是  
    ...
    - **密碼字元集計數下限**  
      強式 PIN 或密碼所需的複雜元素類型 (大寫和小寫字母、數字和標點符號) 數目。 PIN 會強制執行的桌上型電腦和行動裝置的下列行為： 1-數字只有 2-數字和小寫字母都是必要的 3-小寫字母、 數字和大寫英文字母所需。 在電腦 Microsoft 帳戶及網域帳戶中不受支援。 4 - 數字、小寫字母、大寫字母及特殊字元為必要項。 在電腦中不受支援。 預設值為 1。 
  ...
  - **不得重複使用以前用過的密碼**  
    指定可以將多少密碼儲存於無法使用的記錄中。 值會包含使用者的目前密碼。 例如，以設定*1*使用者不能重複使用其目前的密碼，選擇新密碼時。 如果設定為*5*表示使用者無法設定新密碼，向其目前的密碼，或任何其先前的四個密碼。
    
    **預設**：24  

- **避免投影片放映**  
  在 [電腦設定] 中停用鎖定畫面投影片放映設定，來避免在鎖定畫面上放映投影片。 根據預設，使用者可啟用在鎖定機器後執行投影片放映。 如果您啟用此設定，使用者無法修改投影片放映設定，在 [電腦設定]，並沒有投影片放映可以開始。
...
- **Internet Explorer 網際網路區域只允許核准的網域使用 TDC ActiveX 控制項**  
  此原則設定控制，如果使用者可以執行 TDC ActiveX 控制項在網站上。 如果您啟用此原則設定，則不會執行來自此區域網站的 TDC ActiveX 控制項。 如果您停用此原則設定，則會執行來自此區域所有網站的 TDC ActiveX 控制項。
...
- **Internet Explorer 略過不常見的檔案相關的 smartscreen 篩選工具警告**  
...
- **Internet Explorer 限制區域登入選項**  
  ...
  - *只在內部網路區域自動登入*-使用此選項可查詢使用者的使用者識別碼與密碼的其他區域中。 詢問使用者之後，即可在工作階段的其餘部分以無訊息模式使用這些值。 
  - *使用目前的使用者名稱和密碼的自動登入*-使用此選項來嘗試登入使用 Windows NT 挑戰回應 （也稱為 NTLM 驗證）。 如果伺服器支援 Windows NT 挑戰回應，登入使用使用者的網路使用者名稱和密碼登入。 如果伺服器不支援 Windows NT 挑戰回應，則會請使用者提供使用者名稱和密碼。 
...
- **Internet Explorer 內部網路區域不會執行反惡意程式碼不受 Active X 控制項**此原則設定可讓您決定對 ActiveX 控制項，若要檢查其是否安全地載入頁面上，Internet Explorer 是否要執行反惡意程式碼的程式。 如果您啟用此原則設定，Internet Explorer 將不會使用您的反惡意程式碼程式，來檢查是否可以安全建立 ActiveX 控制項的執行個體。 如果您停用此原則設定，Internet Explorer 一律會使用您的反惡意程式碼程式，來檢查是否可以安全建立 ActiveX 控制項的執行個體。 如果您未設定此原則設定，Internet Explorer 將不會使用您的反惡意程式碼程式，來檢查是否可以安全建立 ActiveX 控制項的執行個體。 使用者可以透過 Internet Explorer [安全性] 設定來開啟或關閉此行為。
...
- **Internet Explorer 網際網路區域登入選項**  
  此原則設定可讓您管理登入選項的設定。 如果您啟用此原則設定，即可從下列登入選項中進行選擇。 [匿名登入] 可停用 HTTP 驗證並只針對 Common Internet File System (CIFS) 通訊協定使用來賓帳戶。 [提示輸入使用者名稱及密碼] 可詢問使用者的使用者識別碼和密碼。 詢問使用者之後，即可在工作階段的其餘部分以無訊息模式使用這些值。 [只在內部網路區域自動登入] 可詢問使用者在其他區域的使用者識別碼和密碼。 詢問使用者之後，即可在工作階段的其餘部分以無訊息模式使用這些值。 使用目前的使用者名稱及密碼來自動登入可嘗試使用 Windows NT 挑戰回應 (也稱為 NTLM 驗證) 進行登入。 如果伺服器支援 Windows NT 挑戰回應，登入使用者的網路使用者名稱和密碼登入的使用。 如果伺服器不支援 Windows NT 挑戰回應，則會請使用者提供使用者名稱和密碼。 如果您停用此原則設定，登入會設定為 [只在內部網路區域自動登入]。 如果您未設定此原則設定，登入就會設定為自動登只在近端內部網路區域中。
...
- **Internet Explorer 網際網路區域透過指令碼更新狀態列**  
  此原則設定可讓您管理指令碼是否可以更新區域內的狀態列。 如果您啟用此原則設定時，指令碼可以更新 [狀態] 列。 如果您停用或未設定此原則設定，則不允許指令碼更新狀態列。
...
- **限制具名管道和共用的匿名存取**  
  啟用時，此安全性設定會限制匿名存取共用與管道的設定: (1) 的具名管道可匿名存取 （2） 可以匿名存取的共用
  
  **預設**： 是  
  
- **NTLM SSP 為主之伺服器的最小工作階段安全性**  
  此安全性設定允許伺服器要求 128 位元加密和/或 NTLMv2 工作階段安全性的交涉。 這些值相依於 LAN Manager 驗證等級安全性設定值。 選項包括： 需要 NTLMv2 工作階段安全性： 如果不交涉訊息的完整性，連接將會失敗。 要求 128 位元加密。 若未交涉增強式加密 (128 位元)，連線將會失敗。
...
- **要求用戶端一律為通訊加上數位簽章**：此安全性設定可決定網域成員起始的所有安全通道流量是否必須經簽章或加密。 當電腦加入網域時，會建立電腦帳戶。 之後，當系統啟動時，就會使用電腦帳戶密碼為其網域建立一個與網域控制站的安全通道。 此安全通道可用來執行 NTLM 傳遞驗證、LSA SID/名稱查閱等作業。 此設定可決定網域成員起始的所有安全通道流量是否符合最低安全性需求。 具體而言，它可決定網域成員啟動的所有安全通道流量是否必須經簽章或加密。 如果啟用此原則，除非交涉所有安全通道流量的簽章或加密，否則不會建立安全通道。 如果此原則已停用，則加密且簽署所有的安全通道流量交涉與網域控制站在此情況下的簽章和加密層級取決於網域控制站的版本，以及下列兩個設定原則： 網域成員: （如果可能） 的安全通道資料加以數位加密網域成員： 安全通道資料 （如果可能） 數位簽名
...
- **系統管理員提高權限提示行為**  
  此原則設定可控制系統管理員的提高權限提示行為。 其選項包括： 
    ...
    - 在安全桌面提示輸入認證：當某項作業需要提高權限時，安全桌面會提示使用者輸入特殊權限使用者名稱和密碼。 如果使用者輸入有效的認證，會利用使用者最高的權限，繼續執行該項作業。 
    - 在安全桌面提示要求同意：當某項作業需要提高權限時，安全桌面會提示使用者選取 [允許] 或 [拒絕]。 如果使用者選取 [允許]，則會利用使用者最高的權限，繼續執行該項作業。 
    - 提示輸入認證：當某項作業需要提高權限時，會提示使用者輸入系統管理使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。 
    - 提示要求同意：當某項作業需要提高權限時，會提示使用者選取 [允許] 或 [拒絕]。 如果使用者選取 [允許]，則會利用使用者最高的權限，繼續執行該項作業。  
    - 提示要求同意非 Windows 二進位檔案：當非 Microsoft 應用程式的某項作業需要提高權限時，安全桌面會提示使用者選取 [允許] 或 [拒絕]。 如果使用者選取 [允許]，則會利用使用者最高的權限，繼續執行該項作業。   
  
  **預設**：在安全桌面提示要求同意
  
- **NTLM SSP 為主之用戶端的最小工作階段安全性**  
  此安全性設定允許用戶端要求 128 位元加密和/或 NTLMv2 工作階段安全性的交涉。 這些值相依於 LAN Manager 驗證等級安全性設定值。 其選項包括：
  - 需要 NTLMv2 工作階段安全性： 如果不交涉 NTLMv2 通訊協定，連線將會失敗。 
  - *需要 128 位元加密*： 如果不交涉的強式加密 （128 位元） 的連接會失敗。
...
- **智慧卡移除行為**  
    此安全性設定可決定從智慧卡讀卡機中移除登入使用者之智慧卡時會發生的情況。 其選項包括：
     - 沒有動作。 
     - *將工作站鎖定*-移除智慧卡時，允許使用者將區域保留、 攜帶智慧卡，並仍維持在受保護的工作階段鎖定工作站。
...
- **封鎖使用空白密碼進行遠端登入**  
  此安全性設定可決定是否可以使用未受密碼保護的本機帳戶，從實體電腦主控台以外的位置登入。 如果啟用，不受密碼保護的本機帳戶必須使用電腦鍵盤，登入。 
...
- **標準使用者提高權限提示行為**  
  ...
  - 自動拒絕提高權限要求：當某項作業需要提高權限時，會顯示可設定的拒絕存取錯誤訊息。 以標準使用者身分執行桌面的企業，可以選擇這項設定，以降低客服中心電話。 
  - 在安全桌面提示輸入認證：當某項作業需要提高權限時，安全桌面會提示使用者輸入不同的使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。 
  - 提示輸入認證：當某項作業需要提高權限時，會提示使用者輸入系統管理使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。  
  
  **預設**：自動拒絕提高權限要求
  
- **需要系統管理員的管理員核准模式**  
  此原則設定可控制電腦的所有使用者帳戶控制 (UAC) 原則設定行為。 如果您變更此原則設定，就必須重新啟動電腦。 其選項包括：   
...
- **使用電池期間睡眠時進入待命狀態**  
  如果 Windows 可以使用待命狀態，讓電腦處於睡眠狀態時，就會管理此原則設定。 如果您啟用或未設定此原則設定，Windows 將會使用待命狀態讓電腦進入睡眠狀態。 如果您停用此原則設定，則不允許待命狀態 (S1-S3)。
...
- **系統開機啟動驅動程式初始化**  
  ...
  - 良好：驅動程式已經過簽署且未遭竄改。  
  - 不良 - 驅動程式已識別為惡意程式碼。 我們建議您不要允許已知不正確的驅動程式，以初始化。 
...
- **電子郵件內容執行類型**  
  此規則會封鎖下列的檔案類型，從正在執行，或啟動從 Microsoft Outlook 或 webmail （例如 Gmail.com 或 Outlook.com） 所示的電子郵件： 可執行檔 （例如.exe、.dll 或.scr） 指令碼檔案 （例如 PowerShell.ps、 VisualBasic.vbs 檔案或 JavaScript 的.js 檔案） 指令碼封存檔案。
  
  **預設**： 區塊
  
- **網路保護類型**  
  此原則可讓您在 Windows Defender 惡意探索防護中，開啟或關閉網路保護 (封鎖/稽核)。 網路保護是 Windows Defender 惡意探索防護的一項功能，可防止使用任何應用程式的員工存取網路釣魚詐騙、裝載入侵程式網站及網際網路上的惡意內容。 這包括防止第三方瀏覽器連線到危險的網站。 值類型是整數。 如果您啟用此設定，則會開啟網路保護，且員工無法將它關閉。 其行為可透過下列選項來控制：[封鎖] 和 [稽核]。 如果您使用 [封鎖] 選項來啟用此原則，則會防止使用者和應用程式連線到危險的網域。 您可以看到此 Windows Defender 資訊安全中心中的活動。 如果您使用 [稽核] 選項來啟用此原則，則不會防止使用者/應用程式連線到危險的網域。 不過，您仍然會在 Windows Defender 資訊安全中心看到此活動。 如果您停用此原則，則不會防止使用者/應用程式連線到危險的網域。 您不會在 Windows Defender 資訊安全中心看到任何網路活動。 如果您未設定此原則，封鎖網路預設會停用。
...
- **Defender 潛在的垃圾應用程式動作**  
  Windows Defender 防毒軟體中潛在的垃圾應用程式 (PUA) 防護功能可以識別 PUA，並防止將它下載並安裝到您網路中的端點。 這些應用程式不會視為病毒、惡意程式碼或其他類型的威脅，但可能會在端點上執行對其效能或使用造成不良影響的動作。 PUA 也可能是指那些評價不佳的應用程式。 典型的 PUA 行為包含： 各種類型的軟體將廣告插入至網頁瀏覽器的驅動程式和登錄最佳化程式偵測到問題，要求付款，若要修正錯誤，但保留端點 （也稱為 「 進行任何變更或最佳化rogue 防毒 」 方案）。 這些應用程式可以提高風險的惡意程式碼感染您的網路，會造成惡意程式碼感染，就很難識別，並可能會浪費中清除 應用程式的 IT 資源。  
...
- **Ink 工作區**  
  ...
  - *未設定*-Ink 工作區功能為開啟狀態，以及使用者可以使用它在鎖定畫面上。  

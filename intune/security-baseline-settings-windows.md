...
- **受保護模式的 Internet Explorer ActiveX 控制項**  
  此原則設定可防止 ActiveX 控制項在啟用加強的受保護模式時，於受保護模式下執行。 當使用者所安裝的 ActiveX 控制項與加強的受保護模式不相容，而且網站嘗試載入該控制項時，Internet Explorer 會通知使用者，並讓他們選擇在一般受保護模式下執行網站。 此原則設定可停用此通知，並強制所有網站都在加強的受保護模式下執行。 加強的受保護模式在 64 位元版本的 Windows 上使用 64 位元處理序，藉此提供額外的保護來防範惡意網站。 針對至少執行 Windows 8 的電腦，加強的受保護模式也會限制 Internet Explorer 可從登錄和檔案系統讀取的位置。 當啟用加強的受保護模式，而且使用者遇到網站所嘗試載入的 ActiveX 控制項與加強的受保護模式不相容情況時，Internet Explorer 會通知使用者，並讓他們選擇針對該特定網站停用加強的受保護模式。 如果您啟用此原則設定，Internet Explorer 就無法讓使用者選擇停用加強的受保護模式。 所有受保護模式的網站都會在加強的受保護模式下執行。 如果您停用或未設定此原則設定，Internet Explorer 會通知使用者，讓他們選擇在一般受保護模式下執行具有不相容 ActiveX 控制項的網站。 此為預設行為。
  - **預設**：停用  
...
- **Internet Explorer 可防止管理 SmartScreen 篩選工具**  </br>
  此原則設定可防止使用者管理 SmartScreen 篩選工具，如果所瀏覽的網站已知會透過「網路釣魚」意圖詐騙個人資訊，或已知會裝載惡意程式碼，此工具會警告使用者。 如果您啟用此原則設定，將不會提示使用者開啟 SmartScreen 篩選器。 不在篩選器允許清單上的所有網址都會自動傳送給 Microsoft，而不會提示使用者。 如果您停用或未設定此原則設定，則會在初次執行體驗期間提示使用者決定是否要開啟 SmartScreen 篩選工具。
  - **預設**：啟用  
...
- **標準使用者提高權限提示行為**  
  此原則設定可控制標準使用者的提高權限提示行為。 0 - 自動拒絕提高權限要求：當某項作業需要提高權限時，會顯示可設定的拒絕存取錯誤訊息。 以標準使用者身分執行桌面的企業，可以選擇此設定，以降低客服中心電話。 1 - 在安全桌面提示輸入認證：當某項作業需要提高權限時，安全桌面會提示使用者輸入不同的使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。 3 (預設) - 提示輸入認證：當某項作業需要提高權限時，會提示使用者輸入系統管理使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。
  - **預設**：自動拒絕提高權限要求
...
- **偵測應用程式安裝，並提示提高權限**  
  此原則設定可控制電腦的應用程式安裝偵測行為。 其選項包括：已啟用：(預設) 偵測到需要提高權限的應用程式安裝套件時，會提示使用者輸入系統管理使用者名稱和密碼。 如果使用者輸入有效的認證，會利用可用的權限，繼續執行該項作業。 已停用：不會偵測應用程式安裝套件，也不會提示提高權限。 企業執行標準使用者桌面和使用者代理安裝技術，如群組原則軟體安裝或系統管理伺服器 (SMS) 時，應該停用此原則設定。 在這種情況下，就不需要偵測安裝程式了。
  - **預設**：是

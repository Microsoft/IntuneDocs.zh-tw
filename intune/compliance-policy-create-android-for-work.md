...
- **破解的裝置**：選擇 [封鎖] 以將已破解 (已進行越獄) 的裝置標示為不符合規範。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。
- **裝置須等於或低於裝置威脅等級**：使用此設定以採用來自 Lookout MTP 解決方案的風險評估作為符合規範的條件。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。 若要使用此設定，請選擇允許的威脅等級：
  - **安全**：這是最安全的選項，意謂著裝置不能有任何威脅。 如果在裝置上偵測到任何等級的威脅，即評估為不符合規範。
  - **低**︰如果只有低等級的威脅，會將裝置評估為符合規範。 任何更高等級的威脅都會使裝置處於不相容狀態。
  - **中**︰當裝置上的威脅為低等級或中等級時，系統會將裝置評估為符合規範。 如果在裝置上偵測到高等級的威脅，即判斷為不符合規範。
  - **高**：這是最不安全的選項，因為它允許所有威脅等級。 如果此解決方案只用於報告用途，則此設定可能很實用。
- **已設定 Google Play Services**：**要求**安裝並啟用 Google Play 服務應用程式。 Google Play 服務可允許安全性更新，而且是 Google 認證裝置上許多安全性功能的基層相依服務。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。
- **最新的安全性提供者**：**要求**最新的安全性提供者必須能夠保護裝置免受已知弱點的威脅。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。
...
- **最小 OS 版本**：當裝置不符合最小 OS 版本需求時，會回報為不符合規範。 會顯示如何升級的資訊連結。 終端使用者可以選擇升級其裝置，之後便可以存取公司資源。
- **最大 OS 版本**：當裝置使用的 OS 版本比規則中的版本新時，系統會封鎖它對公司資源的存取。 而且，系統會要求使用者連絡其 IT 管理員。在規則變更為允許該作業系統版本之前，此裝置將無法存取公司資源。

## <a name="system-security-settings"></a>系統安全性設定

### <a name="password"></a>密碼

- **需要密碼來解除鎖定行動裝置**：**要求**使用者輸入密碼，然後才能存取裝置。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。 此設定會套用在裝置層級。 如果您只需要在工作設定檔層級要求密碼，則請使用設定原則。 請參閱 [Android 企業裝置組態設定](device-restrictions-android-for-work.md)。
- **密碼長度下限**：輸入使用者密碼必須包含的數字或字元數目下限。
...
- **在停止活動最少幾分鐘後必須輸入密碼**：輸入使用者必須重新輸入密碼之前的閒置時間。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。
- **密碼到期 (天數)**：選取密碼到期，必須建立新密碼的天數。
- **避免重複使用以前用過的密碼次數**：輸入不可重複使用的最近密碼數目。 使用此設定以限制使用者建立先前使用過的密碼。

### <a name="encryption"></a>加密

- **裝置上的資料儲存區加密**：選擇 [需要] 以將裝置上的資料儲存區加密。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。 

  您不需要進行此設定，因為 Android 工作設定檔裝置會強制執行加密。

### <a name="device-security"></a>裝置安全性

- **封鎖來自不明來源的應用程式**：選擇此選項以**封鎖**來源已啟用 [安全性] > [未知來源] 的裝置 (在 Android 4.0 - Android 7.x 上可支援；Android 8.0 和更新版本則不支援)。 當您選擇 [未設定]  (預設值) 時，則不會評估此設定是否符合規範。
...
- **公司入口網站應用程式執行階段完整性**：選擇 [需要] 以確認 [公司入口網站] 應用程式符合下列所有需求：

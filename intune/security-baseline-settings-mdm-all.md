...
description: 請參閱您可以使用 Microsoft Intune 管理的不同 Windows MDM 安全基準版本的預設值和可用設定。
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 09/06/2019
...
> [!NOTE]  
> 在2019年6月，「5月2019」範本的*MDM 安全性基準*已發行為正式推出（不在預覽階段）。 此版本的安全性基準會取代 prvious 基準，這是*2018 年10月的 MDM 安全性基準*。  在2019版之前建立的設定檔不會更新，以反映5月2019版本中的設定和值。  雖然您無法根據預覽範本來建立新的設定檔, 您可以編輯並繼續使用先前根據預覽範本建立的設定檔。 

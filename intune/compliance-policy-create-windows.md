...
本文列出及描述您可在 Intune 中 Windows 10 及更新版本裝置上設定的不同相容性設定。 作為您行動裝置管理 (MDM) 解決方案的一部分，請使用這些設定來要求 BitLocker、設定最低及最高的作業系統、使用 Windows Defender 進階威脅防護 (ATP) 設定風險層級等。
...
- [Support Tip: Using Device Health Attestation Settings as Part of Your Intune Compliance Policy ](https://techcommunity.microsoft.com/t5/Intune-Customer-Success/Support-Tip-Using-Device-Health-Attestation-Settings-as-Part-of/ba-p/282643) (支援提示：使用裝置健全狀況證明設定，作為您 Intune 相容性原則的一部分)
...
Surface Hub 使用 **Windows 10 及更新版本**平台。 Surface Hub 支援合規性和條件式存取。 若要在 Surface Hub 上啟用這些功能，我們建議您在 Intune 中[啟用 Windows 10 自動註冊](windows-enroll.md) (需要 Azure Active Directory (Azure AD))，並瞄準 Surface Hub 裝置作為裝置群組。 Surface Hub 需要加入 Azure AD，合規性和條件式存取才能運作。

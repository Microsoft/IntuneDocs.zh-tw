...
ms.locfileid: "71724915"
---
# <a name="how-to-configure-the-microsoft-intune-company-portal-app"></a>如何設定 Microsoft Intune 公司入口網站應用程式

[!INCLUDE [azure_portal](../includes/azure_portal.md)]

使用者可以從 Microsoft Intune 公司入口網站存取公司資料及執行一般工作，例如註冊裝置、安裝應用程式，以及尋找向 IT 部門尋求協助的資訊。 此外，公司入口網站應用程式可讓使用者安全地存取公司資源。 公司入口網站應用程式會提供數個不同的頁面，例如 [首頁]、[應用程式]、[應用程式詳細資料]、[裝置] 和 [裝置詳細資料]。 若要在公司入口網站中快速尋找應用程式，您可以在 [應用程式] 頁面上篩選應用程式。

> [!IMPORTANT]
> 若要支援 Google 的 Firebase 雲端通訊 (FCM)，您必須將 Android 公司入口網站應用程式更新到最新版本。 如需詳細資訊，請參閱新功能 - [將您的 Android 公司入口網站應用程式更新到最新的版本](../fundamentals/whats-new.md#update-your-android-company-portal-app-to-the-latest-version-)。
...
當使用者從公司入口網站安裝 iOS 應用程式時，他們會收到提示。 當 iOS 應用程式連結至 App Store、連結至大量採購方案 (VPP)，或連結至企業營運系統 (LOB) 應用程式時，就會發生這種情況。 提示可讓使用者接受此動作，或允許應用程式的管理。 提示字元會顯示您的公司名稱，或當您的公司名稱無法使用時，將會顯示**公司入口網站**。 
...
|欄位名稱|長度上限|詳細資訊|
|---|---|---|
|**連絡人姓名** | 40 | 此姓名會顯示在 [說明及支援]  頁面中。 |
|**電話號碼** | 20 | 此連絡電話號碼會顯示在 [說明及支援]  頁面，讓員工能夠連絡您以尋求支援。 |
|**電子郵件地址**| 40 | 此連絡地址會顯示在 [說明及支援]  頁面中。 您必須輸入有效的電子郵件地址，格式為 `alias@domainname.com`。 |
|**網站名稱**| 40 | 這是支援網站的 URL 所顯示的易記名稱。 若只指定支援網站 URL 而未指定易記名稱，將會在公司入口網站的 [說明及支援]  頁面上顯示 [移至 IT 部門網站]。 |
|**網站 URL**| 150 | 如果想讓使用者參考您的支援網站，請在這裡指定 URL。 URL 的格式必須為 `https://www.contoso.com`。 如果您沒有指定 URL，公司入口網站的 [說明及支援]  頁面上將不會顯示支援網站的任何內容。 |
| **其他資訊**| 120 | 在 [說明及支援]  頁面上顯示。 |
...
您可以在受控 iOS 裝置上自訂為您的組織顯示的隱私權聲明。 此訊息會列出您的組織無法在受控 iOS 裝置上查看或執行的項目。

在[公司入口網站自訂]   > [裝置管理與隱私權訊息]  底下，您可以：

- 接受 [預設]  以使用顯示的清單，或
- 選擇 [自訂]  ，以自訂您的組織無法在受控 iOS 裝置上查看或執行的項目。 您可以使用 [arkdown](https://daringfireball.net/projects/markdown/) \(英文\) 來加入項目符號、粗體、斜體和連結。
...
## <a name="user-self-service-device-actions-from-the-company-portal"></a>公司入口網站的使用者自助裝置動作

使用者可以透過公司入口網站應用程式或網站，在其本機裝置或遠端裝置上執行動作。 使用者可以執行的動作會根據裝置平台和設定而有所不同。 在所有情況下，只能由裝置的主要使用者執行遠端裝置動作。
- **淘汰** – 從 Intune 管理移除裝置。 在公司入口網站應用程式和網站中，這會顯示為 [移除]  。
- **抹除** – 此動作會起始裝置重設。 在公司入口網站中，這會顯示為 [重設]  或在 iOS 公司入口網站應用程式中顯示為 [恢復出廠預設值]  。
- **重新命名** – 此動作會變更使用者可以在公司入口網站中看到的裝置名稱。 它不會變更本機裝置名稱，只會變更公司入口網站中的清單。
- **同步處理** – 此動作會使用 Intune 服務起始裝置簽入。 這在公司入口網站中會顯示為 [檢查狀態]  。
- **遠端鎖定** – 這會鎖定裝置，需要 PIN 才能將其解除鎖定。
- **重設密碼** – 此動作用來重設裝置密碼。 在 iOS 裝置上，將會移除密碼，而且使用者將需要在 [設定] 中輸入新的密碼。 在支援的 Android 裝置上，Intune 會產生新的密碼，並暫時在公司入口網站中顯示。
- **金鑰復原** – 此動作用來復原加密裝置的加密金鑰。

### <a name="self-service-actions"></a>自助服務動作

某些平台和設定不允許自助裝置動作。 下表提供自助服務動作的進一步詳細資料：
...
<sup>(2)</sup> 只有透過 Web 入口網站才能使用 MacOS 的金鑰復原。<br>
<sup>(3)</sup> 如果使用裝置註冊管理員註冊，則會停用所有遠端動作。<br>
<sup>(4)</sup> 重新命名只會變更公司入口網站應用程式或網站中的裝置名稱，而不是在裝置上。<br>
...
<sup>(7)</sup> 淘汰和抹除不適用於 Android Enterprise 裝置擁有者情節 (COPE、COBO、COSU)。<br> 

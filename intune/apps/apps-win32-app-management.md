...
4. 在 [新增]  應用程式窗格中，從提供的下拉式清單中選取 [Windows 應用程式 (Win32)]  。
    ...
    ![[新增應用程式] 窗格 - [新增類型] 下拉式方塊的螢幕擷取畫面](./media/apps-win32-app-management/apps-win32-app-01.png)

### <a name="step-2-upload-the-app-package-file"></a>步驟 2：上傳應用程式套件檔案

1. 在 [新增應用程式]  窗格中，選取 [新增套件檔案]  來選取檔案。 應用程式套件檔案窗格隨即顯示。

    ![[應用程式套件檔案] 窗格的螢幕擷取畫面](./media/apps-win32-app-management/apps-win32-app-02.png)
...
3. 若要設定 [解除安裝命令]  ，請新增完整解除安裝命令列來根據應用程式的 GUID 將它解除安裝。 

    例如：`msiexec /x “{12345A67-89B0-1234-5678-000001000000}”`

4. 將 [安裝行為]  設定為 [系統]  或 [使用者]  。
...
5. 若要設定 [裝置重新啟動行為]  ，請選取下列其中一個選項：
    - **根據傳回碼決定行為**：選擇此選項來根據[傳回碼](~/apps/apps-win32-app-management.md#step-7-configure-app-return-codes)組態設定來重新啟動裝置。
...
3. 按一下 [新增]  以顯示 [新增需求規則]  窗格，並設定其他需求規則。 選取 [需求類型]  以選擇您要用於決定如何驗證需求的規則類型。 需求規則可能會以檔案系統資訊、登錄值或 PowerShell 指令碼為基礎。 
...
5. 在 [指派]  窗格中，選取以根據使用者或裝置進行指派。 當您選擇指派時，您也可以選擇 [使用者體驗]  。 [使用者體驗]  可讓您設定 [使用者通知]  、[重新啟動寬限期]  、[可用性]  ，以及 [安裝期限]  。 如需詳細資訊，請參閱＜設定 Win32 應用程式可用性和通知＞  。
6. 選取 [確定]  來完成所包含的群組選取項目。
7. 如果您想要將任何使用者群組排除，使他們不受此應用程式指派影響，請選取 [排除群組]  。
8. 在 [新增群組]  窗格中，選取 [確定]  。
9. 在應用程式 [指派]  窗格中，選取 [儲存]  。
...
1. 在 Intune 中，選取 [應用程式]   > [所有應用程式]  以檢視您的已新增用戶端應用程式清單。 
...
4. 從應用程式窗格中，選取 [指派]   > [加入群組]  。 
5. 將 [指派類型]  設定為 [必要]  。 請注意，應用程式可用性可以根據指派類型設定。 [指派類型]  可以是 [必要]  、[適用於已註冊的裝置]  ，或 [解除安裝]  。
6. 選取 [包含的群組]  來判斷會將應用程式指派給哪一個使用者群組。 將會顯示 [指派]  窗格。
7. 將 [要求所有使用者都安裝此應用程式]  設定為 [是]  。
...
8. 若要修改 [使用者體驗]  選項，請選取 [編輯]  。
9. 在 [編輯指派]  窗格中，將 [使用者通知]  設定為 [顯示所有快顯通知]  。 請注意，您可以將 [使用者通知]  設定為 [顯示所有快顯通知]  、[顯示電腦重新開機的快顯通知]  ，或是 [隱藏所有快顯通知]  。
10. 將 [應用程式可用性]  設定為 [特定日期與時間]  ，然後選取您的日期與時間。 此日期與時間會指定將應用程式下載到使用者裝置上的時機。 
11. 將 [應用程式安裝期限]  設定為 [特定日期與時間]  ，然後選取您的日期與時間。 此日期與時間會指定將應用程式安裝到使用者裝置上的時機。 針對相同的使用者或裝置進行多個指派時，系統會根據最早的可能時間來挑選應用程式安裝期限的時間。
12. 按一下 [重新啟動寬限期]  旁邊的 [已啟用]  。 重新啟動寬限期會在裝置上的應用程式安裝完成後立即開始。 停用時，裝置可以在不發出警告的情況下重新啟動。 <br>您可以自訂下列選項：

...
1. 登入 [Intune](https://go.microsoft.com/fwlink/?linkid=2090973)。
2. 在 [Intune]  刀鋒視窗中，選取 [用戶端應用程式]   > [應用程式]  。
3. 從清單中選取現有的 [Windows 應用程式 (Win32)]  。 
4. 從應用程式刀鋒視窗中，選取 [指派]   > [加入群組]  。 
5. 將 [指派類型]  設定為 [必要]  。 請注意，應用程式可用性可以根據指派類型設定。 [指派類型]  可以是 [必要]  、[適用於已註冊的裝置]  ，或 [解除安裝]  。
6. 選取 [包含的群組]  來判斷會將應用程式指派給哪一個使用者群組。 將會顯示 [指派]  刀鋒視窗。
...
9. 在 [編輯指派]  刀鋒視窗中，將 [使用者通知]  設定為 [顯示所有快顯通知]  。 請注意，您可以將 [使用者通知]  設定為 [顯示所有快顯通知]  、[顯示電腦重新開機的快顯通知]  ，或是 [隱藏所有快顯通知]  。

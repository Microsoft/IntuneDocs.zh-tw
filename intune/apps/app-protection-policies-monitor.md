...
應用程式保護資料的保留期間為 90 天。 過去 90 天內簽入至 MAM 服務的任何應用程式執行個體，都將包含於 [應用程式保護狀態] 報告中。 應用程式執行個體是唯一的使用者 + 應用程式 + 裝置。 
...
![[Intune 行動應用程式管理] 窗格上的 [摘要] 磚](./media/app-protection-policies-monitor/app-protection-user-status-summary.png)
...
>[!NOTE]
> [上次同步處理] 欄代表主控台內使用者狀態報告與應用程式保護原則[可匯出的 .csv 報告](https://docs.microsoft.com/intune/app-protection-policies-monitor#export-app-protection-activities-to-csv)兩者中相同的值。 差異是 2 個報告中同步的些許延遲。 
>
> [上次同步處理] 中參考的時間是 Intune 上次看到「應用程式執行個體」的時間。 應用程式執行個體是應用程式 + 使用者 + 裝置的唯一組合。 當終端使用者啟動應用程式時，它可能會或可能不會在啟動時聯繫 Intune 應用程式保護服務，取決於它上次的簽入時間。 此文件有助於釐清[應用程式保護原則簽入的重試間隔時間-in](https://docs.microsoft.com/en-us/intune/app-protection-policy-delivery)。 因此，若終端使用者未在上次簽入間隔 (通常是 30 分鐘的主動使用) 之內使用該特定應用程式，而且他們啟動應用程式，則：
>
> - 應用程式保護原則可匯出的 .csv 報告將會有 1 分鐘 (通常；最小值) 到 30 分鐘 (由 Intune Reporting 使用之 SQL 彙總所提供的實際最大 SLA) 之內的最新時間。
> - 使用者狀態報告將會立即有最新的時間。
>
> 例如，考慮已設定目標的已授權終端使用者，它在下午 12:00 啟動了受保護的應用程式：
> - 若這是第一次登入，則表示終端使用者之前未登入 (未主動使用)，也就是說他們未向 Intune 進行應用程式執行個體註冊。 一旦登入，他們將會取得新的應用程式執行個體註冊，而且系統會立即他們簽入，而不會有任何擱置中的連線問題；針對未來的簽入則列出相同的時間延遲。因此，上次同步處理時間在使用者狀態報告中將回報為下午 12:00，而在應用程式保護原則報告中則會回報為下午 12:01 (最壞情況則下午 12:30)。 
> - 若他們剛啟動應用程式，則回報的上次同步處理時間將取決於他們上次簽入的時間。
...
[詳細檢視] 會顯示錯誤訊息、發生錯誤時存取的應用程式、受影響的裝置作業系統平台和時間戳記。 這裡會報告具有透過「SafetyNet 裝置證明」條件式啟動檢查旗標之裝置的使用者，以及 Google 所報告的原因。

### <a name="users-with-potentially-harmful-apps"></a>具潛在有害應用程式的使用者
詳細的檢視會顯示使用者、應用程式套件識別碼、應用程式是否已啟用 MAM、威脅類別、電子郵件、裝置名稱及時間戳記。 這裡會報告具有透過「需要對應用程式進行威脅掃描」條件式啟動檢查旗標之裝置的使用者，以及 Google 所報告的威脅類別。 如果這份報告中列出的應用程式是透過 Intune 部署的，請連絡該應用程式的應用程式開發人員，並/或移除該應用程式，以使其無法指派給您的終端使用者。 
...
- **應用程式報表**：除了選取平台和應用程式之外，此報表提供兩種不同的應用程式保護狀態，供您在產生報表之前選取。 狀態可以是 [受保護]  或 [不受保護]  。
  ...
  - 非受控 MAM 活動的使用者狀態 ([不受保護]  )：這份報表會依每個使用者列出目前非受控之啟用 MAM 的應用程式活動。 可能的發生原因如下︰
    ...
    ![使用者的 [應用程式報告] 刀鋒視窗螢幕擷取畫面，其中包含 3 個應用程式的詳細資料](./media/app-protection-policies-monitor/MAM-reporting-4.png)

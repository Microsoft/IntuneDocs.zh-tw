...
| 檔案下載服務意外停止。 將會在下次裝置同步時重試原則。 (0xC7D1507C) | OS 在下載程序完成之前就已將它終止。 當裝置電力偏低或下載時間太長時，就會發生此錯誤。 此錯誤只會針對 DA 案例傳回。 針對 KNOX 案例，系統不會提示使用者安裝，這會透過無訊息方式完成。 手動同步裝置或等待24小時，並檢查狀態。 |
| 應用程式無法卸載。 (0xc7d14fb8) | 此錯誤是一般卸載失敗。 作業系統未指定為何無法卸載應用程式。 某些管理應用程式無法直接卸載。 檢查以確定應用程式可以手動卸載，如果卸載失敗，則收集公司入口網站記錄檔。 |
| 用於升級的應用程式安裝 APK 檔不符合裝置上目前應用程式的簽章。 (0xc7d14fb7) | Android OS 的限制是要求升級版本的簽署憑證與用來簽署現有版本的憑證完全相同。 如果開發人員無法使用相同的憑證來簽署新版本，您將需要卸載現有的應用程式，並重新部署新的應用程式，而不是升級現有的應用程式。 |
| 終端使用者已取消應用程式安裝。 (0xc7d14fb9) | 教育使用者接受 Intune 部署的應用程式，並在出現提示時安裝應用程式。 |
| 因為處理序在安裝期間重新啟動，所以應用程式的安裝已取消。 (0xc7d14fbc) | 應用程式安裝進程已由作業系統終止，或裝置已重新開機。 如果再次發生此錯誤，請重試安裝，並收集公司入口網站記錄檔。 |
| 無法安裝應用程式安裝 APK 檔，因為它尚未簽署。 (0xc7d14fb6) | 根據預設，Android OS 需要簽署應用程式。 請確定應用程式在部署前已簽署。 |
...
| 此應用程式為受控，但已過期或被使用者移除。 (0x87D13B66) | 使用者已明確卸載應用程式，或應用程式已過期但無法下載，或應用程式偵測不符合裝置的回應。 此外，發生此錯誤的原因可能是 iOS 9.2.2 平台 Bug。 |
...
| 使用者已拒絕所提供的應用程式安裝項目。 (0x87D13B62) | 在初始應用程式安裝期間，使用者已按下 [取消]。 要求使用者下一次接受安裝要求。 |
...
| 授權指派失敗，發生 Apple 錯誤「沒有任何 VPP 授權剩餘」（0x87d13b7e） | 此行為是設計使然。 若要解決此問題，請購買額外的 VPP 授權，或從不再以為目標的使用者回收授權。 |
| 應用程式安裝失敗12024：未知的原因。 (0x87d13b6e) | Apple 尚未提供我們足夠的資訊來判斷安裝失敗的原因。 沒有任何要報告的內容。 |
| 必要的應用程式設定原則不存在，請確定原則是以相同的群組為目標。 (0x87d13b7f) | 應用程式需要應用程式設定，但未將應用程式設定設為目標。 系統管理員應該確定應用程式的目標群組也具有以群組為目標的必要應用程式設定。 |
| 裝置 VPP 授權僅適用于 iOS 9.0 以上的裝置。 (0x87d13b69) | 將受影響的 iOS 裝置升級至 iOS 9.0 +。 |
| 應用程式已安裝在裝置上，但未受管理。 (0x87d13b8f) | 只有 LOB 應用程式才會發生此錯誤。 應用程式已安裝在 Intune 外部。 若要解決此錯誤，請從裝置卸載應用程式。 下一次裝置同步處理時，裝置應該會從 Intune 安裝應用程式。 |
| 使用者拒絕的應用程式管理（0x87d13b68） | 要求使用者接受應用程式管理。 |
| 未知的錯誤。 (0x87d1279d) | IOS 儲存應用程式發生此錯誤，但錯誤案例不明。 |
| 應用程式的最新版本無法從較舊的版本更新。 (0x87D13B9D) | 如果已安裝並管理應用程式，但裝置上的版本不正確，就會顯示此錯誤訊息。 這種情況包括裝置已收到更新應用程式的命令，但尚未安裝並回報新版本的情況。 部署升級之後，裝置的第一次簽入將會報告此錯誤，並且會在裝置回報新版本已安裝或因不同錯誤而失敗之前進行。  |

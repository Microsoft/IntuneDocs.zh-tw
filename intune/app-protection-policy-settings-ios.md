...
| **允許應用程式將資料傳送到其他應用程式** | 指定可以接收這個應用程式資料的應用程式： <ul><li>**無**：不允許將資料傳送到任何應用程式 (包括其他受原則管理的應用程式)。</li><li> **受原則管理的應用程式**：只允許傳送至其他受原則管理的應用程式。</li><li>**具有 OS 共用的原則受控應用程式**：僅允許將資料傳輸到其他原則受控的應用程式，以及將檔案傳輸至已註冊裝置上的其他 MDM 受控應用程式。<p><p>**注意︰***[具有 OS 共用的原則受控應用程式]  值僅適用於 MDM 註冊裝置。如果此設定以未註冊裝置上的使用者為目標，則適用於**原則受控應用程式**值的行為*。<p><p></li><li>**具有開啟方式/共用的原則受控應用程式篩選**：僅允許傳輸到其他原則受控應用程式，並篩選 OS [開啟方式/共用] 對話方塊，以僅顯示原則受控應用程式。<p><p>**注意︰***若要設定 [開啟方式/共用] 對話方塊的篩選，則同時需要作為檔案/文件來源的應用程式，以及可開啟此檔案/文件來使用 Intune SDK for iOS 8.1.1 版或更新版本的應用程式。*<p><p></li><li>**所有應用程式**：允許傳送到任何應用程式。</li></ul><br>此外，設定為 [原則受控應用程式] 或 [無]，則會封鎖允許焦點搜尋在應用程式中搜尋資料的 iOS 9 功能。 <p><p>此原則也適用於 iOS 通用連結。  一般 Web 連結則是由 [限制與其他應用程式的 Web 內容傳輸] 原則設定所管理。 <p> 有一些 Intune 可預設允許資料傳送至其中的豁免應用程式和服務。 此外，如果您需要允許資料傳送至不支援 Intune 應用程式的應用程式，您可以建立您自己的豁免設定。 請參閱[資料轉送豁免](#data-transfer-exemptions)以取得詳細資訊。 <br><br> 預設值 = [所有應用程式] |
| **允許應用程式接收來自其他應用程式的資料** | 指定可將資料傳送至這個應用程式的應用程式： <ul><li>**無**：不允許從任何應用程式 (包括其他受原則管理的應用程式) 傳送資料。</li><li>**受原則管理的應用程式**：只允許從其他受原則管理的應用程式傳送。</li><li>**所有應用程式**：不允許從任何應用程式傳送資料。</li><li>**所有包含傳入組織資料的應用程式**：不允許從任何應用程式傳送資料。 將沒有使用者身分識別的所有傳入資料視為組織中資料。 資料將使用由 `IntuneMAMUPN` 設定定義的 MDM 註冊使用者身分識別來進行標記。<p><p>**注意︰***[所有包含傳入組織資料的應用程式] 值僅適用於 MDM 註冊裝置。如果此設定以未註冊裝置上的使用者為目標，則適用於**任意應用程式**值的行為。*</li></ul> 有一些 Intune 可以允許從中進行資料傳輸的豁免應用程式和服務。 如需應用程式和服務的完整清單，請參閱[資料傳輸豁免](#data-transfer-exemptions)。 在未註冊的 iOS 裝置上，啟用多重身分識別 MAM 的應用程式會忽略這項原則，並允許所有內送資料。<br><br> 預設值 = [所有應用程式]|
| **不可進行另存新檔** | 選取 [是]，在此應用程式中停用 [另存新檔] 選項。 如果您想要允許使用 [另存新檔]，請選取 [否]。 <br><br> 預設值 = 否 <br><br>***注意：** Microsoft Excel、OneNote、Outlook、PowerPoint 和 Word 支援此設定。協力廠商和 LOB 應用程式也可以支援此設定。* <br><br> 設定為 [是] 時，您可以設定下列設定： <ul><li>**選取要用於儲存公司資料的儲存體服務**：使用者可以儲存到幾個選取的服務 (商務用 OneDrive、SharePoint 和本機存放區)。 會封鎖所有其他服務。 <br><br> 預設值 = **選取 0** |
...
| 設定 | 如何使用 |  
|---------|------------| 
| **最低 OS 版本** | 指定使用此應用程式的最低 iOS 作業系統。 「動作」包括： <br><ul><li>**警告** - 如果裝置上的 iOS 版本不符合需求，使用者將會看見通知。 此通知可以關閉。 <br><br> </li></ul> <ul><li>**封鎖存取** - 如果裝置上的 iOS 版本不符合需求，將會禁止使用者存取。</li></ul> <ul><li>**抹除資料** - 與應用程式建立關聯的使用者帳戶會從裝置抹除。  </li></ul> </li></ul> ***注意：** 應用程式需要有 Intune SDK 7.0.1 版或更新版本。* |

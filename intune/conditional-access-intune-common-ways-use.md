...
1.  使用者嘗試存取裝載於 Exchange 內部部署 2010 SP1 或更新版本的公司電子郵件。

2.  如果裝置不受 Intune 管理，將會封鎖它對電子郵件的存取。 Intune 會將封鎖通知傳送至 EAS 用戶端。

3.  EAS 會收到封鎖通知、將該裝置移到隔離，並傳送含有補救步驟的隔離電子郵件，其中包含連結，讓使用者能夠註冊其裝置。
...
若行動裝置已安裝 Mobile Threat Defense 代理程式，該代理程式就可將合規性狀態訊息傳回 Intune，以回報是否已在行動裝置本身發現威脅。

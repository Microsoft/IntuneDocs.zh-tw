...
3.  在 [建立設定檔] 刀鋒視窗中，鍵入設定檔的名稱，將平台選取為 [Windows 10 和更新版]，然後將設定檔類型選取為 [版本升級]。 按一下 [進行設定]。

5. 在 [版本升級] 刀鋒視窗中，在 [Edition to upgrade to] (將版本升級至) 上，選取 [Windows 10 Holographic for Business]。 在 [授權檔案] 上，瀏覽至提供給您的 XML 授權檔案並加以選取。
...
2. 在 [指派] 刀鋒視窗中，選取您想要包含與排除原則的使用者群組與裝置。

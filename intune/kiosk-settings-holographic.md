...
- **應用程式**：新增要在 kiosk 裝置上執行的應用程式。 請記住，您可以新增數個應用程式。
  ...
  - **新增市集應用程式**： 選取現有的應用程式新增，或部署到 Intune，作為[用戶端應用程式](apps-add.md)，包括 LOB 應用程式。 如果您沒有任何列出的應用程式，Intune 支援許多[應用程式類型](apps-add.md)您[新增至 Intune](store-apps-windows.md)。

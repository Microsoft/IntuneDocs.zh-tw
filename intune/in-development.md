...
當 Intune 中的 Apple VPP 權杖已同步處理時, 您使用 Apple Business Manager 購買的 macOS 應用程式將會顯示在主控台中。 您可以使用主控台為群組指派、撤銷和重新指派裝置和使用者型授權。 Microsoft Intune 可協助您管理已購買供公司使用的 VPP 應用程式, 方法是:
- 從應用程式市集報告授權資訊。
- 追蹤您已經使用多少個授權。
...
您將能夠安裝 Web 應用程式, 這可讓您使用 macOS 公司入口網站, 將 web 上 URL 的快捷方式新增至 Dock。 使用者可以在 macOS 公司入口網站中, 從 web 應用程式的 [應用程式詳細資料] 頁面存取 [**安裝**] 動作。 如需**Web 連結**應用程式類型的詳細資訊, 請參閱[將應用程式新增至 Microsoft Intune](apps-add.md)。
...
若要查看目前的設定, 請移至 [ [IOS Kiosk (僅限受監督)] 設定](device-restrictions-ios.md#kiosk-supervised-only)。
...
如需這些設定的詳細資訊, 請參閱[顯示或隱藏應用程式 (僅限受監督)](device-restrictions-ios.md#show-or-hide-apps-supervised-only)。
...
- 適用于完全受控 Android 的 SCEP 憑證可用於以裝置擁有者身分管理之裝置上的 cert 驗證。 公司設定檔裝置上已支援 SCEP 憑證。  使用適用于裝置擁有者的 SCEP 憑證時, 您將能夠: <!-- 5227935 -->
    ...
    - 連結 SCEP 憑證以進行驗證的電子郵件設定檔 (透過 AppConfig)

---
title: "iOS MAM 原則設定 | Microsoft Docs"
...
您可以為 Azure 入口網站中 [所有設定] 刀鋒視窗上的應用程式保護原則，[設定](create-and-deploy-mobile-app-management-policies-with-microsoft-intune.md)本主題內所述的原則設定。
...
| **選取要用於儲存公司資料的儲存體服務** | 使用者可以儲存到幾個選取的服務 (商務用 OneDrive、SharePoint 和本機存放區)。 將會封鎖所有其他服務。 | 商務用 OneDrive 和 SharePoint |

...
### 安全性設定
所有設定適用於 iOS 7.1 和更新版本。

|設定名稱|詳細資料|
|----------------|-------|
|**需要密碼來解除鎖定行動裝置**|指定使用者是否需要輸入密碼才能存取其裝置。|
|**所需的密碼類型**|指定必要密碼的類型，例如只可包含數字，或必須是英數字元等等。|
|**密碼所需的複合字元數**|這會指定密碼中必須包含的符號字元數 (例如 **#** 或 **@**)。|
|**最小密碼長度**|指定密碼的字元數下限。|
|**允許簡單密碼**|允許簡單密碼 (like ‘0000’ 和 ‘1234’)。|
...
|設定名稱|詳細資料|
|----------------|-------|
|**允許擷取螢幕畫面**|允許使用者擷取螢幕的內容作為影像。|
|**允許在鎖定畫面時使用控制中心**|當裝置鎖定時，控制是否可以存取控制中心應用程式。|
|**允許在鎖定畫面時使用通知檢視**|允許使用者在未解除鎖定裝置的情況下存取通知檢視。|
|**允許在鎖定畫面時使用今日檢視**|當裝置鎖定時，控制是否可以檢視通知。|
...
|**允許 Handoff，以繼續執行其他裝置上的活動**|Handoff 可讓您繼續執行在另一個 iOS 的 iOS 裝置上或 Mac OS X 裝置上所啟動的工作。|
...
|**允許自動填滿**|使用者可以變更瀏覽器中的自動完成設定。|
|**允許快顯封鎖程式**|啟用或停用瀏覽器的快顯封鎖程式。|
|**允許 Cookie**|允許裝置網頁瀏覽器使用 Cookie。|
...
|設定名稱|詳細資料|
|----------------|-------|
|**允許應用程式市集**|允許裝置存取應用程式市集。|
|**需要密碼來存取應用程式存放區**|要求使用者輸入密碼，才可以瀏覽應用程式市集。|
|**允許應用程式內購買**|允許從執行中應用程式進行市集購買。|
|**允許在其他未受管理的應用程式中使用受管理文件**|允許在任何應用程式中檢視公司文件。<br>**範例︰**您想要防止使用者將檔案從 OneDrive 應用程式儲存至 Dropbox。 將此設定設定為否。 裝置收到原則後 (例如重新啟動後)，它就不再允許儲存。|
|**允許在其他受管理應用程式中使用未受管理的文件**|允許在公司受管理應用程式中檢視任何文件。|
|**允許視訊會議**|允許在裝置上進行視訊會議應用程式 (例如 Facetime)。|
|**允許媒體市集中的成人內容**|允許裝置存取市集中評分為成人的內容。|
|**允許使用者從 iBook Store 下載標示為 [成人作品] 的內容**|允許使用者下載分類為 [成人作品] 的書籍。|
...
|----------------|-------|
|**允許相機**|指定是否可以使用裝置上的相機|
|**須有配對密碼，才能傳出 AirPlay 要求**|Airplay 可讓您將內容串流至其他 Apple 裝置。 使用此設定需要有配對密碼，才能連線到其他裝置。|
...
|**啟用旁白**|啟用或停用協助工具設定 [VoiceOver]，以大聲讀出裝置顯示畫面上的文字。|
|**啟用旁白調整**|啟用或停用旁白調整，以讓您調整 VoiceOver 功能 (例如，大聲讀出螢幕上文字的速度)。|
|**啟用縮放**|啟用或停用 [縮放] 協助工具設定，以讓您使用觸控來縮放裝置顯示畫面。|
|**啟用縮放調整**|啟用或停用縮放調整，以讓您調整縮放功能。|
|**啟用色彩對換**|啟用或停用 [反向顏色] 協助工具設定，以調整顯示畫面來協助視力受損的使用者。|
|**啟用色彩對換調整**|啟用或停用色彩對換調整，以讓您調整反向顏色功能。|
|**啟用輔助觸控**|啟用或停用 [AssistiveTouch] 協助工具設定，協助使用者執行可能很難執行的螢幕手勢。|
|**啟用輔助觸控調整**|啟用或停用輔助觸控調整，以讓您調整輔助觸控功能。|
|**啟用語音選取**|啟用或停用 [朗讀所選區域] 協助工具設定，以大聲讀出您選取的文字。|
> [!NOTE]
...
|設定名稱|詳細資料|
|----------------|--------------------|
|**允許裝置在受監督的模式時使用啟用鎖**|在受監督的 iOS 裝置上將啟用鎖啟用。|

### 監督
下列設定可以在處於受監督模式且執行 iOS 7.1 和更新版本的裝置上設定。
...
|**允許在裝置上使用清除所有內容及設定的選項**|允許使用者使用選項，清除裝置上的所有內容及設定。|
|**允許使用者啟用裝置設定中的限制**|允許使用者在裝置上設定裝置限制 (分級保護控制)|
|**允許 Spotlight 搜尋傳回網際網路上的結果**|讓 Spotlight 搜尋連線到網際網路，提供進一步的結果。|
|**允許使用 Game Center 應用程式**|允許使用 Game Center 應用程式。|
|**允許主機配對，以控制 iOS 裝置所能配對的裝置**|主機配對可讓系統管理員控制 iOS 7 裝置所能配對的裝置。|

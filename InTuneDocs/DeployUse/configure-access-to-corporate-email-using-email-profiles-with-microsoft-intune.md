...
[註解]：<> 後續三個段落中的被動建構是必要的，直到重複偵測程序完成清除 PM。
...
| ----------- | --------------- |
    ...
    |**選取用戶端憑證以進行用戶端驗證 (識別憑證)** (Samsung KNOX 和 iOS)|選取先前建立的用戶端 SCEP 憑證，以用來驗證 Exchange 連線。 如需如何使用 Intune 中憑證設定檔的詳細資訊，請參閱[使用憑證設定檔保護資源存取](secure-resource-access-with-certificate-profiles.md)。 只有在驗證方法是 [憑證] 時，才會顯示此選項。|

...
description: "Intune 有數種方式可協助您保護使用 Intune 用戶端管理的電腦，包括設定 Windows Firewall 防火牆設定。"
keywords: 
author: robstackmsft
manager: arob98
ms.date: 07/19/2016
...
這些原則設定可在連線到網域 (例如，在工作場所中)、私人 (信任) 網路 (例如家用網路) 或不受信任的公用網路 (例如咖啡廳) 的受管理電腦上啟用 Windows 防火牆。 所有這些設定的預設值都是 [是]，亦即最安全的值。 

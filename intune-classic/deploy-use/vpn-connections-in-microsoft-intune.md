...
設定名稱  |詳細資訊  
---------|---------
**名稱**     |輸入 VPN 設定檔的唯一名稱，協助您在 Intune 主控台中識別該 VPN 設定檔。         
**描述**     |提供概述 VPN 設定檔的描述以及協助您找到它的其他相關資訊。         
**VPN 連線名稱 (對使用者顯示)**     |指定 VPN 設定檔的名稱。 這是使用者在其裝置的可用 VPN 連線清單中看到的名稱。         
**連線類型**     |  請在下列項目中選取 VPN 設定檔要使用的連線類型：[Cisco AnyConnect (Windows 8.1 或 Windows Phone 8.1 無法使用)]、[Pulse Secure]、[Citrix]、[F5 Edge Client]、[SonicWall Mobile Connect]、[CheckPoint Mobile VPN]。
...
**登入群組或網域**|指定您想要連線之登入群組或網域的名稱。 只有在連線類型為 [SonicWall Mobile Connect] 時，才會顯示此選項。
